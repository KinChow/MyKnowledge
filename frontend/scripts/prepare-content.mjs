...
function resolveLinks(md){
  md=md.replace(/\[\[([A-Za-z0-9][A-Za-z0-9_-]*)\]\]/g,(m,id)=>validIds.has(id)?`[${titleOf.get(id)}](/wiki/${id}/)`:m);
  md=md.replace(/\]\(([^)\s]+?)\)/g,(m,url)=>{ if(/^(https?:|\/|#|mailto:)/i.test(url))return m; const mm=url.match(/([^/]+?)\.md(#.*)?$/i); if(!mm)return m; const id=mm[1]; return validIds.has(id)?`](/wiki/${id}/${mm[2]||''})`:m; });
  return md;
}
