...
def run_manifest(tmp_path: Path, manifest: dict) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        ["node", str(SCRIPT), str(path)], capture_output=True, text=True, check=False
    )
...
def test_empty_public_projection_is_valid(tmp_path: Path):
    result = run_manifest(
        tmp_path,
        {"schema_version": "public-projection/v1", "projection": "public", "items": []},
    )
    ...
    for body_path in ("practice/questions/q.json", "%2e%2e/wiki/q.md"):
        result = run_manifest(
            tmp_path,
            {
                ...
                "projection": "public",
                "items": [{**base, "body_path": body_path}],
            },
        )
        assert result.returncode != 0
...
def test_prepare_content_rejects_confirmation_precondition_drift(tmp_path: Path):
    ...
    event = {
        ...
        "actor_id": "alice",
        "decision": "approve",
        "release_input_sha256": "sha256:old",
        "reviewed_content_sha256": "sha256:old",
...
def test_projection_prepare_and_graph_build_multi_page_fixture(tmp_path: Path):
    ...
    for ident, title, links in (("one", "One", ["two"]), ("two", "Two", [])):
        ...
        event = {
            "schema_version": "public-release-confirmation/v1",
            "event_id": f"event-{ident}",
            "operation_id": f"op-{ident}",
            "target_ref": {
                "vault_id": "public",
                "object_type": "wiki",
                "object_id": ident,
    ...
    assert "中文 attention" in (frontend / "src/content/docs/one.md").read_text(
        encoding="utf-8"
    )
    assert "English transformer" in (frontend / "src/content/docs/two.md").read_text(
        encoding="utf-8"
    )
    graph = subprocess.run(
        ["node", str(frontend / "scripts" / "build-graph.mjs")],
        cwd=frontend,
        capture_output=True,
        text=True,
    )
...
def test_leak_gate_reports_input_scope_and_rejects_practice(tmp_path: Path):
    ...
    result = subprocess.run(
        ["node", str(script), "--scope", "input-tree", str(tmp_path)],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_validate_build_rejects_pagefind_count_mismatch(tmp_path: Path):
    """pagefind 闭包对象是 catalog（Starlight 文档页），非全部 html。"""
    target = tmp_path / "dist"
    (target / "pagefind").mkdir(parents=True)
    (target / "index.html").write_text("<html></html>", encoding="utf-8")
    (target / "graph").mkdir(parents=True)
    (target / "graph" / "index.html").write_text(
        "<html></html>", encoding="utf-8"
    )  # 自定义页不计入 pagefind
    (target / "pagefind" / "pagefind-entry.json").write_text(
        json.dumps({"languages": {"zh-cn": {"page_count": 2}}}), encoding="utf-8"
    )
    (tmp_path / "public" / "generated").mkdir(parents=True)
    (tmp_path / "public" / "generated" / "catalog.json").write_text(
        json.dumps({"items": [{"id": "one", "route": "one"}]}), encoding="utf-8"
    )  # catalog 1 ≠ pagefind 2
    (tmp_path / "public" / "generated" / "graph.json").write_text(
        json.dumps({"nodes": [{"id": "one"}], "edges": []}), encoding="utf-8"
    )
    ...
    assert result.returncode != 0 and "pagefind_catalog_not_closed" in result.stderr
...
def test_validate_build_requires_exact_sitemap_catalog_closure(tmp_path: Path):
    ...
    generated = tmp_path / "public" / "generated"
    ...
    (target / "sitemap.xml").write_text(
        "<urlset><url><loc>/</loc></url><url><loc>/graph/</loc></url><url><loc>/one/</loc></url></urlset>",
        encoding="utf-8",
    )
    result = subprocess.run(
        ["node", str(script), str(target)],
