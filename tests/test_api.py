...
def test_cli_query_matches_api_query_result(tmp_path: Path):
    ...
    api_result = client.get(
        "/api/query", params={"q": "离线", "scope": "public"}
    ).json()
    cli = subprocess.run(
        [sys.executable, "-m", "tools.cli", "query", "离线", "--root", str(tmp_path)],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_cli_read_and_backlinks_use_public_projection(tmp_path: Path):
    ...
    read = subprocess.run(
        [sys.executable, "-m", "tools.cli", "read", "one", "--root", str(tmp_path)],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert links.returncode == 0 and json.loads(links.stdout)["items"] == [
...
def test_create_app_loads_public_projection_when_items_are_not_injected():
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert [item["object_ref"]["object_id"] for item in result.json()["items"]] == [
            "one"
        ]
...
def test_public_post_requires_capability_but_public_get_remains_anonymous():
    client = TestClient(create_app(items=ITEMS, capability_token="token"))
    assert (
        client.get("/api/query", params={"q": "离线", "scope": "public"}).status_code
        == 200
    )
...
def test_get_query_rejects_unknown_parameters():
    client = TestClient(create_app(items=ITEMS, capability_token="token"))
    response = client.get(
        "/api/query", params={"q": "离线", "scope": "public", "include_private": "true"}
    )
...
def test_private_scope_requires_capability():
    ...
    assert (
        client.post(
            "/api/retrieve", json={"query": "内部", "scope": "local"}
        ).status_code
        == 401
    )
    assert (
...
def test_capability_audience_is_checked_when_supplied():
    client = TestClient(create_app(items=ITEMS, capability_token="token"))
    headers = {
        "X-MyKnowledge-Capability": "token",
        "X-MyKnowledge-Audience": "other-service",
    }
    response = client.post(
        "/api/retrieve", headers=headers, json={"query": "内部", "scope": "local"}
    )
...
def test_capability_scope_registry_is_enforced():
    ...
    response = client.get(
        "/api/vault/check", headers={"X-MyKnowledge-Capability": "token"}
    )
    assert response.status_code == 403
...
def test_citation_replay_api_is_read_only_and_capability_scoped():
    ...
    assert (
        client.post(
            "/api/citation/replay", json={"citation": citation, "snapshot": snapshot}
        ).status_code
        == 401
    )
    response = client.post(
...
def test_public_read_and_backlinks(tmp_path: Path):
    ...
    (wiki / "consumer.md").write_text(
        "See [/wiki/target](/wiki/target)", encoding="utf-8"
    )
    ...
    assert links.json()["items"] == [
        {"vault_id": "public", "object_type": "wiki", "object_id": "consumer"}
    ]
...
def test_non_public_read_requires_capability_even_when_vault_unavailable(
    tmp_path: Path,
):
    client = TestClient(create_app(root=tmp_path, capability_token="token"))
...
def test_capability_token_rotates_with_secure_permissions(tmp_path: Path):
    ...
    assert (
        client.post(
            "/api/retrieve",
            headers={"X-MyKnowledge-Capability": token_one},
            json={"query": "x", "scope": "local"},
        ).status_code
        == 403
    )
...
def test_cross_origin_post_is_rejected_before_capability_check():
    ...
    response = client.post(
        "/api/retrieve",
        headers={"Origin": "https://evil.example", "X-MyKnowledge-Capability": "token"},
        json={"query": "离线", "scope": "public"},
    )
    assert response.status_code == 403
...
def test_non_loopback_host_is_rejected():
    ...
    response = client.post(
        "/api/retrieve",
        headers={"Host": "remote.example", "X-MyKnowledge-Capability": "token"},
        json={"query": "离线", "scope": "public"},
    )
    assert response.status_code == 403
...
def test_retrieve_enforces_policy_vault_limit():
    ...
    response = client.post(
        ...
        json={
            "query": "x",
            "scope": "public",
            "vault_ids": [f"v-{i}" for i in range(17)],
        },
    )
    assert response.status_code == 400
...
def test_api_passes_vault_allowlist_into_retriever_before_search():
    ...
    response = client.post(
        "/api/retrieve",
        headers={"X-MyKnowledge-Capability": "token"},
        json={"query": "x", "scope": "private", "vault_ids": ["private"]},
    )
    assert response.status_code == 200
...
def test_request_body_limit_is_fail_closed():
    ...
    response = client.post(
        "/api/retrieve",
        headers={"Content-Length": str(1_048_577), "X-MyKnowledge-Capability": "token"},
        content=b"{}",
    )
...
def test_validate_endpoint_requires_capability_and_reuses_wiki_validator(
    ...
    response = client.post(
        "/api/validate/public/wiki/target",
        headers={"X-MyKnowledge-Capability": "token"},
    )
    assert response.status_code == 200
...
def test_private_vault_read_and_backlinks_are_owner_scoped(tmp_path: Path):
    ...
    assert links.status_code == 200
    assert links.json()["items"] == [
        {"vault_id": "private", "object_type": "wiki", "object_id": "consumer"}
    ]
...
def test_practice_api_exposes_deterministic_mode_and_llm_unavailable(tmp_path: Path):
    ...
    deterministic = client.post(
        "/api/practice/q-one/answer",
        params={"scope": "local", "scoring_mode": "deterministic"},
        headers=headers,
        json="包含核心",
    )
    assert (
    ...
    unavailable = client.post(
        "/api/practice/q-one/answer",
        params={"scope": "local", "scoring_mode": "llm"},
        headers=headers,
        json="x",
    )
    assert (
        unavailable.status_code == 200
        and unavailable.json()["reason"] == "provider_unavailable"
    )
...
def test_practice_review_api_persists_fsrs_card_state(tmp_path: Path):
    ...
    client = TestClient(create_app(root=tmp_path, capability_token="token"))
    response = client.post(
        "/api/practice/q-review/review",
        params={"scope": "local", "rating": 3},
        headers={"X-MyKnowledge-Capability": "token"},
    )
    assert response.status_code == 200
    body = response.json()
    ...
    if body.get("state") == "scheduled":
        ...
        assert (
            QuestionStore(tmp_path).load("q-review")["review_state"]["card_id"]
            == body["card"]["card_id"]
        )
...
def test_include_sources_attaches_references_not_silently_ignored(tmp_path: Path):
    ...
    client = TestClient(create_app(root=tmp_path, capability_token="token"))
    result = client.get(
        "/api/query", params={"q": "事后回顾", "include_sources": "true"}
    ).json()
    assert result["items"], result
    assert result["items"][0].get("sources") == ["aar"]  # 附带 source 引用而非静默忽略
    archive = client.get(
        "/api/query", params={"q": "事后回顾", "include_archive": "true"}
    ).json()
