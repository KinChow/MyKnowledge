...
class FetcherTests(unittest.TestCase):
    def test_redirect_is_rechecked_and_response_limit_is_enforced(self):
        class Response:
            def __init__(self, status, body=b"", location=None): self.status, self.body, self.location = status, body, location
            def getheader(self, name): return self.location if name == "Location" else "identity"
            def read(self, size=-1): return self.body[:size]
        class Connection:
            responses = [Response(302, location="http://example.com/next"), Response(200, b"too-large")]
