...
def test_inventory_hash_changes_when_input_changes():
    with tempfile.TemporaryDirectory() as d:
        root = Path(d); (root / "docs").mkdir(); p = root / "docs" / "a.md"
        p.write_text("# A\n", encoding="utf-8"); first = inventory(root)["input_tree_sha256"]
        p.write_text("# B\n", encoding="utf-8"); assert inventory(root)["input_tree_sha256"] != first
