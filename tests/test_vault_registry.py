...
class VaultRegistryTests(unittest.TestCase):
    ...
    def test_backup_not_configured_warning_has_safe_next_action(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            warning = next(
                item
                for item in BackupManager(root, manifest).status()["backup_summary"][
                    "warning"
                ]
                if item["vault_id"] == "private"
            )
    ...
    def test_private_public_projection_permission_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            report = VaultRegistry(root, manifest).check()
            self.assertEqual(
                report["vaults"][0]["reason"], "public_projection_confidentiality"
            )
    ...
    def test_reference_rejects_cross_vault_even_when_target_exists(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            result = VaultRegistry(public, manifest).validate_reference(
                "public", "private", "source", "s"
            )
    ...
    def test_effective_confidentiality_propagates_from_upstream(self):
        self.assertEqual(
            VaultRegistry.effective_confidentiality("public", ["internal"]), "internal"
        )
        self.assertEqual(
            VaultRegistry.effective_confidentiality("public", ["public"]), "public"
        )
    ...
    def test_object_index_keeps_same_ids_separate_by_owner(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                set(index), {("public", "wiki", "same"), ("private", "wiki", "same")}
            )
            self.assertEqual(
                {item["availability"] for item in index.values()}, {"available"}
            )
    ...
    def test_local_projection_merges_same_ids_with_owner_and_private_scope(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                refs, {("public", "wiki", "same"), ("private", "wiki", "same")}
            )
            private_only = VaultRegistry(public, manifest).local_projection("private")
            self.assertEqual(
                {item["vault_id"] for item in private_only["items"]}, {"private"}
            )
            self.assertNotIn(
                "secret",
                json.dumps(
                    {"schema_version": private_only["schema_version"], "items": []}
                ),
            )
    ...
    def test_backup_bundle_rejects_intermediate_payload_symlink(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                self.assertEqual(
                    BackupManager.verify_bundle(bundle)["error_code"],
                    "entry_path_symlink",
                )
            finally:
                shutil.rmtree(bundle, ignore_errors=True)
    ...
    def test_backup_bundle_restores_to_empty_checkout_and_cleans_on_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            created = manager.create_manifest("public")
            bundle = root.parent / (root.name + "-bundle")
            target = root.parent / (root.name + "-restored")
            try:
                manager.export_bundle(root / created["path"], bundle)
    ...
    def test_restored_bundle_verification_rejects_extra_target_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            target = root.parent / (root.name + "-target-verify")
            try:
                manager.export_bundle(root / created["path"], bundle)
                restored = manager.restore_bundle(bundle, target)
                self.assertEqual(restored["state"], "restored")
    ...
    def test_backup_rejects_manifest_with_rehashed_tampered_durable_record(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            subprocess.run(["git", "init", "-q", str(root)], check=True)
            manager = BackupManager(root)
            operation = root / "audit" / "operations" / "op-one.json"
            ...
            data["manifest_sha256"] = (
                ...
                .sha256(
                    __import__(
                        "tools.common", fromlist=["canonical_json"]
                    ).canonical_json(
                        {k: v for k, v in data.items() if k != "manifest_sha256"}
                    )
                )
    ...
    def test_verified_manifest_restores_to_empty_checkout(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                next(v for v in status["vaults"] if v["vault_id"] == "public")[
                    "backup_state"
                ],
                "verified",
            )
    ...
    def test_practice_entries_are_owner_scoped_and_restored(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            question = {
                ...
                "wiki_claim": {"wiki_id": "w", "claim_id": "c"},
                "prompt": "secret",
                "options": None,
    ...
    def test_restored_practice_semantics_are_verified(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            manager = BackupManager(
                root, extra_verifiers={"practice": practice_integrity_check}
            )
            manifest = manager.create_manifest("public")
            with tempfile.TemporaryDirectory() as out:
                target = Path(out) / "checkout"
    ...
    def test_restore_cleans_partial_checkout_after_write_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with tempfile.TemporaryDirectory() as out:
                target = Path(out) / "checkout"
                original = __import__(
                    "tools.backup", fromlist=["atomic_write"]
                ).atomic_write
                calls = {"count": 0}
