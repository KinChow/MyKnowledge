...
def _event(object_id: str, content: str, evidence: str):
    return {
        "schema_version": "public-release-confirmation/v1", "event_id": f"event-{object_id}",
        "operation_id": f"op-{object_id}", "target_ref": {"vault_id": "public", "object_type": "wiki", "object_id": object_id},
        "target_vault": "public", "actor_type": "human", "actor_id": "alice", "decision": "approve",
        "release_input_sha256": "sha256:input", "reviewed_content_sha256": content, "reviewed_evidence_sha256": evidence,
        "leak_gate_report_sha256": "sha256:leak", "leak_gate_report_scope": "input-tree",
        "reason": "Reviewed public knowledge release", "confirmation_nonce": f"nonce-{object_id}",
    }
...
def test_public_projection_generator_requires_matching_confirmation(tmp_path: Path):
    ...
    reports = {
        "one": {"valid": True, "object_ref": {"vault_id": "public", "object_type": "wiki", "object_id": "one"}, "derived": {"public_publishable": True}, "hashes": {"content_sha256": "sha256:one", "evidence_sha256": "sha256:e1"}},
        "two": {"valid": True, "object_ref": {"vault_id": "public", "object_type": "wiki", "object_id": "two"}, "derived": {"public_publishable": True}, "hashes": {"content_sha256": "sha256:two", "evidence_sha256": "sha256:e2"}},
    }
    ...
    assert {item["id"] for item in json.loads((tmp_path / "queries" / "public" / "manifest.json").read_text())["items"]} == {"one"}
    assert {item["object_id"] for item in result["skipped"]} == {"two"}


def test_public_projection_generator_does_not_emit_private_or_unconfirmed_items(tmp_path: Path):
    wiki = tmp_path / "wiki"; wiki.mkdir()
    (wiki / "private.md").write_text("secret", encoding="utf-8")
    reports = {"private": {"valid": True, "object_ref": {"vault_id": "private", "object_type": "wiki", "object_id": "private"}, "derived": {"public_publishable": False}, "hashes": {}}}
    result = PublicProjectionGenerator(tmp_path, FakeValidator(reports)).generate()
