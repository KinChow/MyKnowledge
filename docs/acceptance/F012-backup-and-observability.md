...
- `python -m tools.cli backup export-bundle --root R --manifest M --target B` 将已校验 manifest 和 payload 物化到显式离线 bundle；`restore-bundle --root R --manifest B --target D --target-vault-id V` 从 bundle 恢复到显式目标目录并校验 owner。两条命令均不自动推导 `verified`，也不执行 checkout/reset/commit/push。
