...
- 状态：Retired（ADR-0019，2026-09-15）——本文件描述的全部机制（`inventory` 清单、`migrate` 批量迁移、`confirm-apply` 确认门、`migration-record/v1` 重放、基于 `WriteOperation` 的受控 rollback）已从代码中删除：`tools/inventory_legacy.py`、`tools/migrate_legacy.py`、`tools/reposition.py`、`tools/write_operation.py` 与 CLI `inventory`/`migrate`/`reposition`/`write`/`confirm-apply` 一并退场，测试 `tests/test_inventory.py`、`tests/test_migration.py`、`tests/test_reposition.py` 已删除。保留正文是为了记录"曾经实现过"的事实，**下方断言均已失效**，不得作为验收依据。
- 退场依据：ADR-0019（Accepted）§3 删除清单（四类人工确认事件 + operation 状态机 + per-vault 锁）。存量内容的一次性迁移已完成其使命（464 篇 source / 208 篇 wiki 已落盘，161 篇降级落位在 `content/working/`），继续保留迁移工具的成本高于其剩余价值。
- 当前边界：本条不再是验收对象。若将来再次需要存量迁移，应按 ADR-0017 的统一动词（`source`/`wiki` 领域函数）+ 一次落盘重写，而不是复活 `operation` 状态机。

## 退场说明

- 全部 3 条 AC（`AC-F010-001` / `AC-F010-002` / `AC-F010-003`）连同其下所有增量证据段一并退场；每段以 `> 已失效` 标注。
- **例外**：`## DOCX 抽取边界增量证据` 中引用的 `tests/ingest/test_extractor.py::ExtractorTests::test_docx_extractor_does_not_fallback_to_binary_text` 仍存活 —— 它钉的是**抽取器**行为（DOCX 不得按 UTF-8 fallback），属于 F001 的领域，只是历史上被登记在本文件。同一节中依赖 `inventory` 的断言已失效。
- 与 F011/F013 的分工：跨 Vault copy/move 与 working 层降级落位分别在 `F011-private-vault.md`、`F013-layers-and-channels.md` 中标注退场，不在本文件重复。
...
> 部分失效（ADR-0019，2026-09-15）：本节中依赖 `inventory`/`migrate` 的断言已随工具删除而失效；`ExtractorTests::test_docx_extractor_does_not_fallback_to_binary_text` 仍存活（抽取器行为属于 F001 领域）。
