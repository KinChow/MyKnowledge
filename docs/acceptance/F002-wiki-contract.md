...
- Feature：F002
- 相关规范：WIKI
- 状态：Implemented（2026-08-26；41/41 测试通过，其中 F002 12 条）
- 测试运行：`.venv/bin/python -m pytest tests/test_f002.py -v`
...
- 失败时不变量：未通过校验的对象不得发布；
- 自动化级别：Unit。
- 对应测试：`test_legal_knowledge_wiki`
...
- Then：校验失败并给出字段级错误；
- 自动化级别：Unit。
- 对应测试：`test_missing_source_rejected`
...
- 对应测试：`test_derived_fields_rejected`（7 个字段逐字段断言）、`test_legal_knowledge_wiki`（hash 可复现）
- 当前状态：通过。`FORBIDDEN_DERIVED_FIELDS`（14 个字段）先于 schema 校验，返回 `derived_field_mismatch`；`content_sha256`（canonical_body）与 `evidence_sha256`（解析后 evidence 含 resolved_object_ref）由工具计算。
...
- 自动化级别：Unit。
- 对应测试：`test_status_axis_combinations`、`test_snapshot_drift_derives_stale`、`test_validation_report_drives_states`
...
- 自动化级别：Unit/Integration。
- 对应测试：`test_owner_vault_reference_resolution`
...
- 对应测试：`test_publication_combo_rejected`、`test_status_axis_combinations`（published 组合）、`test_private_publishable_with_confirmation`
...
- 自动化级别：Unit/Integration。
- 对应测试：`test_executable_schema_rejects_unknown_and_wrong_version`
...
- 测试：52/52 通过（2026-08-26，Python 3.14.2 / pytest 8.4.2，F001 29 + F002 12 + 结构守卫 3 + 崩溃注入 2）
