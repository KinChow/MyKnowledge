...
本轮 FSRS 恢复重放调查（2026-08-27）：`py-fsrs` 6.3.2（MIT，<https://github.com/open-spaced-repetition/free-spaced-repetition-scheduler>）的 `Card.to_dict()/from_dict()` 是官方状态序列化边界；Anki 24.x（GPL-3.0，<https://github.com/ankitects/anki>）将 card 状态与 review log 分离。替代方案是恢复后重建新 Card，会丢失 due/stability/lapses 等调度事实并造成复习间隔跳变。验收采用原始题目 FSRS review state 经 owner-scoped manifest 恢复后，由 adapter 继续 review，比较恢复前 Card ID/state 与恢复后连续调度结果；缺少 FSRS 依赖仍必须返回 `provider_unavailable`，不得伪造重放成功。
