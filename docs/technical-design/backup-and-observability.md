...
本轮恢复 CLI 归属调查（2026-08-27）：Restic/Borg 的 restore 命令都把来源 archive/repository 与显式目标分开；替代方案是仅以目标路径推断 owner，会在同名 private/public bundle 时产生跨 Vault 恢复。CLI `restore-bundle` 现在强制 `--target-vault-id`，并委托 `restore_bundle_to_vault()` 在创建目标前做 owner equality 校验；缺失或错配均 fail-closed。离线命令不联网、不写 remote/credential，升级只增加必填参数。
...
测试覆盖多 Vault 独立状态、备份目标缺失、备份校验失败、空仓恢复、损坏 snapshot、丢失/篡改 audit 或 confirmation record、单个 Vault 恢复失败不影响其他 Vault、旧 projection 保留和日志脱敏。恢复演练报告记录时间、工具版本、输入 manifest hash、校验结果、恢复后 object/snapshot/evidence/attestation hash 集合、durable record 自校验结果和人工复核人；缺少任一 durable record 必须保持 `failed`/`unconfigured`。`BackupManager.status` 对已配置 Vault 的最近 durable manifest 做 schema/self-hash/entries 结构检查；损坏派生为 `failed`，manifest 存在本身不会提升为 `verified`。
