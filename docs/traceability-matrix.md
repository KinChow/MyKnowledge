...
| QST-001 | F008 | ADR-0008 | [question-and-practice](./technical-design/question-and-practice.md) | AC-F008-001/002/003/004/005/006/007/008/009 | tests/test_question.py + tests/test_api.py + tests/test_vault_registry.py + tests/test_frontend_projection.py（严格 question/v1 未知字段/题型字段、持久化 content hash fail-closed、Wiki/Claim identity 与 hash/批量失效、选择题 option allowlist/简答 rubric/provider、评分记录、FSRS 6.3.2 真实 Card 连续重放与 `fsrs-card/v1`/scheduler version 元数据、practice API scoring mode、owner-scoped 备份恢复、public path-independent question payload leak gate）；完整恢复仍待补 | Implemented（部分） |
