...
| OPS-001 | F004 | ADR-0006 | write-operation-and-locking | AC-F004-001/002/003/004/005/006/010/011 | tests/test_write_operation.py（preview/apply/回滚/commit-intent 显式恢复/fencing/stale lock recovery/durable audit 校验、symlink/hard-link target 拒绝、Preview 后路径竞态结构化失败、private owner checkout 路径绑定）；领域场景待补 | Implemented（部分） |
...
| OPS-002 | F011 | ADR-0002/0006 | [private-vault-submodule](./technical-design/private-vault-submodule.md) | AC-F011-001/002/003/004/008/009/011/014/015/016/019 | tests/test_vault_registry.py + tests/test_api.py + tests/test_vault_transfer.py（Registry、available scope、owner-aware local-projection 合并、同 Vault 冲突 fail-closed 排除、显式跨 Vault copy/move 与 confidentiality/双锁门禁、跨 Vault 同名/冲突、owner-scoped private read/backlinks、public projection 权限一致性）；staging 失败恢复和 projection generator 待补 | Implemented（部分） |
| SKILL-001 | F009 | ADR-0006/0007 | [agent-skill](./technical-design/agent-skill.md) | AC-F009-001/002/003/004/005/006/007/008/009/010/011/012 | tests/test_skill_contract.py + tests/test_skill_runtime.py（action 字段 schema/白名单、危险参数、真实 MCP stdio `tools/list`/`tools/call` 与 capability、preview/apply、public projection query/retrieve/read/backlinks、practice scoring mode 委托、question create validator 路由）；完整 publish/token 生命周期待补 | Implemented（部分） |
...
| SCH-001 | F002 | ADR-0001/0006 | [schema validation](./technical-design/schema-validation.md) | AC-F002-001/003/004/006/007 | tests/test_front_matter.py + tests/validation/test_wiki_schema.py（SafeLoader 重复 key 拒绝、派生字段/未知字段/schema version/类型校验）；跨域组合和 private 挂载待补 | Implemented（部分） |
| QST-001 | F008 | ADR-0008 | [question-and-practice](./question-and-practice.md) | AC-F008-001/002/003/004/005/006/007/008/009 | tests/test_question.py + tests/test_api.py + tests/test_vault_registry.py + tests/test_frontend_projection.py（严格 question/v1 未知字段/题型字段、持久化 content hash fail-closed、Wiki/Claim identity 与 hash/批量失效、选择题 option allowlist/简答 rubric/provider、评分记录、FSRS 6.3.2 真实 Card 连续重放与 `fsrs-card/v1`/scheduler version 元数据、practice API scoring mode、owner-scoped 备份恢复、恢复后 question/review 语义闭包校验、public path-independent question payload leak gate）；完整恢复仍待补 | Implemented（部分） |
