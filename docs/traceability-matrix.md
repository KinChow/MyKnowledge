...
| BAK-001 | F012 | ADR-0002/0006 | [backup-and-observability](./technical-design/backup-and-observability.md) | AC-F012-001/002/003/004/006/007/008 | tools/backup.py + tools/cli.py + tests/test_vault_registry.py（owner-scoped manifest/restore marker、合法 owner metadata/offline bundle 导出校验、`backup export-bundle`/`restore-bundle` 显式 target CLI、外部 target manifest/空仓恢复且不伪造 verified、显式 target_vault_id 与 manifest owner 匹配、恢复后 entry 集合/hash/额外文件校验、practice question/review 语义闭包校验、backup_not_configured 脱敏 next_action、manifest/record 双层篡改校验、失败清理、恢复后 verified 派生）；跨 Vault 完整 object/snapshot/evidence/projection 重建待补 | Implemented（部分） |
