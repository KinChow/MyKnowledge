...
| F008 | 可插拔题目平台、大模型短回合学习和复习 | 消费端 | P1 | F002, F003, F006, F011, F012 | [F008](./acceptance/F008-question-practice.md) |
...
当前与本次方案直接相关的状态：F001/F002/F003 已实现（2026-08-27，F003 LLM 证据审计链路 + corroboration-v1 + 人工确认写入完成）；F004 的写入门禁已 `Retired`（ADR-0019，2026-09-15）：Preview/Apply 协议、operation 状态机、commit-intent 与 per-vault 锁从代码中删除（OPS-001/003/004 在追踪矩阵标 Retired），写入改为一次落盘、审批由 `git commit` 承担；存活的 `confirm`/`release`/`override` 属发布确认与审计复议，不是写入门禁；此前已有通用 writer、retire/purge 门禁和 private owner-root 路径绑定；跨 Vault staging 与领域 writer 统一迁移未闭合；F005 已进入 `Implemented`（projection、SQLite FTS5、fallback 与 Registry owner-aware projection 接入，QMD/完整恢复验收未闭合）；F006 已进入 `Implemented`（FastAPI retrieve/query/ask 基础能力，完整 API 验收未闭合）；F007 已进入 `Implemented`（Astro 工程、静态 graph、确定性 sitemap、leak gate；public projection 与逐页发布确认已对 208 个公开页完成，manifest 与 269 条确认事件已提交并发布到 `kinchow.github.io`；完整 25 AC 浏览器自动化仍待补，见 WEB-001）；F008 已进入 `Implemented`（题目 schema、绑定、评分和 FSRS adapter 基础能力，完整验收未闭合）；F009 已进入 `Implemented`（canonical Skill 基础契约，完整运行验收未闭合）；F010 的迁移工具已 `Retired`（ADR-0019，2026-09-15）：`tools/inventory_legacy.py` / `migrate_legacy.py` 与 `inventory` / `migrate` 命令已删除，MIG-001 在追踪矩阵标 Retired——迁移本身早已跑完（`content/working` 层整层删除并全部消费进 wiki）；F011 已进入 `Implemented`（Registry、owner-aware local projection，完整 Vault 写入/恢复验收未闭合；本仓库为全公开单 vault、无私有 vault，private 远端为 N/A）；F012 已进入 `Implemented`（状态、manifest、隔离恢复与 durable restore marker，外部 target 验收未闭合；本仓库以公开 GitHub remote 为异地副本，加密私有备份为 N/A）。以上均不等同于 `Accepted`。
...
F007 的发布门禁按逐页人工确认（`public-release-confirmation/v1`）+ projection 派生 `public_release` 运作。**本仓库已对全部 208 个公开页完成确认**：`var/queries/public/manifest.json`（208 项 `public_release: true`/`status: published`）与 `release/public-confirmations/`（269 条确认事件）均已提交，前端据此构建并发布到 `kinchow.github.io`。本仓库为**全公开单 vault**——所有内容公开、无私有 vault，公开 GitHub remote 即异地副本，因此 F011 的 private Git remote 与 F012 的加密私有备份对本部署为 **N/A**，`backup_state: unconfigured` 是如实终态而非待办。日后若挂载私有 vault，再按 F011/F012 配置真实远端/加密备份，未配置时必须告警且不能声称恢复链路就绪。
