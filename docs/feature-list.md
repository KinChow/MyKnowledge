...
当前与本次方案直接相关的状态：F005–F012（不含延后的 F008）均为 `Designed`，尚未声称代码 `Implemented` 或 `Accepted`；验收文档中的 `Not Implemented` 是有意保留的事实边界。当前 checkout 还没有 `skills/myknowledge/SKILL.md` 和领域 writer/backend；F009 的 canonical Skill 文件存在性本身是验收门，不得从文档路径的预留描述推断已实现。
