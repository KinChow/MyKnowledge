...
当前与本次方案直接相关的状态：F001/F002/F003 已实现（2026-08-27，F003 LLM 证据审计链路 + corroboration-v1 + 人工确认写入完成）；F004 已进入 `Implemented`（通用 writer，领域验收未闭合）；F005 已进入 `Implemented`（projection/fallback 基础能力，完整 FTS5/QMD 验收未闭合）；F006 已进入 `Implemented`（FastAPI retrieve/query/ask 基础能力，完整 API 验收未闭合）；F008 已进入 `Implemented`（题目 schema、绑定、评分和 FSRS adapter 基础能力，完整验收未闭合）；F009 已进入 `Implemented`（canonical Skill 基础契约，完整运行验收未闭合）；F010 已进入 `Implemented`（legacy inventory 基础能力，Source-first 迁移未闭合）；F011 已进入 `Implemented`（只读 Registry，完整 Vault 验收未闭合）；F012 已进入 `Implemented`（状态与 manifest 基础能力，完整恢复验收未闭合）；F007 仍为 `Designed`，尚未声称代码 `Implemented` 或 `Accepted`；验收文档中的 `Not Implemented` 是有意保留的事实边界。领域 writer/backend 的完整能力和各 Feature 验收门仍需继续补齐。
