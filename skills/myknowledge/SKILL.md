...
- Source/Wiki changes: writes are a single durable landing (no preview/apply
  two-phase); approval is the Git commit itself (ADR-0019). Show the target
  vault, hashes and warnings. Audit and publish still require explicit human confirmation:
  the human runs `python -m tools.cli confirm` to write an operation-confirmation/v1
  event bound to the current content/evidence hashes. The Agent channel cannot
  self-confirm.
