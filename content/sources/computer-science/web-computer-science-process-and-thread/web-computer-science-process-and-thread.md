...
attachments:
- filename: web-computer-science-process-and-thread.html
  kind: document
...
retrieval:
  acquisition: fetch
  resolved_url: https://man7.org/linux/man-pages/man7/pthreads.7.html
...
pthreads(7)          Miscellaneous Information Manual         pthreads(7)
   ...
   LinuxThreads
       ...
       •  In addition to the main (initial) thread, and the threads that
          the program creates using pthread_create(3), the implementation
       ...
       •  Threads do not share process IDs.  (In effect, LinuxThreads
          ...
          visible as separate processes using ps(1).
       ...
       •  When a thread calls execve(2), all other threads are terminated
          ...
          the same PID as the thread that called execve(2): it should
   ...
   NPTL
       ...
       •  Only the main thread is permitted to start a new session using
          setsid(2) (fixed in Linux 2.6.16).
       •  Only the main thread is permitted to make the process into a
          process group leader using setpgid(2) (fixed in Linux 2.6.16).
       •  Threads have distinct alternate signal stack settings.
          However, a new thread's alternate signal stack settings are
          copied from the thread that created it, so that the threads
   ...
   Selecting the threading implementation: LD_ASSUME_KERNEL
       ...
       (i.e., glibc 2.3.*), the LD_ASSUME_KERNEL environment variable can
       ...
       man-pages@man7.org
Linux man-pages 6.18            2026-02-08                    pthreads(7)
