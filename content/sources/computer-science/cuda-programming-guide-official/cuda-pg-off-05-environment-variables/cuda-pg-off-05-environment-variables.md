...
attachments:
- filename: cuda-pg-off-05-environment-variables.html
  kind: document
  media_type: text/html
  role: original
  sha256: sha256:31e56398554a9b94628eff08781ce635f15a728bd7b8e87ad72e91f8c299ecdf
confidentiality: public
domain: computer-science
extractor: trafilatura/2.2.0
id: cuda-pg-off-05-environment-variables
media_type: text/html
origin: external
raw_ref:
  path: archive/raw/31e56398554a9b94628eff08781ce635f15a728bd7b8e87ad72e91f8c299ecdf.html
  sha256: sha256:31e56398554a9b94628eff08781ce635f15a728bd7b8e87ad72e91f8c299ecdf
read_status: retrieved
retrieval:
  acquisition: fetch
  resolved_url: https://docs.nvidia.com/cuda/cuda-programming-guide/05-appendices/environment-variables.html
  url: https://docs.nvidia.com/cuda/cuda-programming-guide/05-appendices/environment-variables.html
schema_version: source/v1
snapshot_sha256: sha256:3e717d6fe7f0ed3542c6a92fc575c9ef94ab506d262a99890bbdc04b55347465
source_type: doc
vault_id: public
---
...
- Integer indices: These correspond to the ordinal number of the GPU in the system, as determined by nvidia-smi , starting from 0. For example, settingCUDA_VISIBLE_DEVICES=2,1 makes device 0 not visible and enumerates device 2 before device 1.
...
- PCI_BUS_ID : The available devices are enumerated by PCI bus ID in ascending order. The PCI bus IDs can be obtained withnvidia-smi --query-gpu=name,pci.bus_id .
Examples:
...
The environment variable controls the behavior of the on-disk Just-In-Time (JIT) compilation cache. Disabling the JIT cache forces PTX to CUBIN compilation for a CUDA application each time it is executed, unless the CUBIN code for the running architecture is found in the binary. The same cache is also used to store finalized cubins, and setting this environment variable will also force finalization of cubins, where necessary.
...
Possible Values:
- 1 : Disables PTX JIT and binary finalization caching.
...
The environment variable specifies the directory path for the Just-In-Time (JIT) compilation cache. The same cache is also used to store finalized cubins.
...
The environment variable specifies the Just-In-Time (JIT) compilation cache size in bytes. Binaries that exceed this size are not cached. If needed, older binaries are evicted from the cache to make room for newer ones. The same cache is also used to store finalized cubins.
...
- Any value other than 0.25x ,0.5x ,2x or4x is interpreted as1x .
Example:
...
This environment variable provides a hint to the CUDA Driver and Runtime to prefer a spin-based locking mode for certain internal host-side synchronization paths. In this mode, internal locks may actively poll for availability rather than blocking and yielding the CPU. This can reduce synchronization latency for short-held locks by avoiding kernel transitions and wakeup overhead, improving fast‑path performance under low to moderate contention (for example, when multiple host threads contend on the same CUDA object or execution context). Spin-based locking does not provide scheduler-assisted fairness or priority inheritance. Under host thread preemption, oversubscription, or mixed workloads, enabling spin-based locking may increase the risk of priority inversion, starvation, or reduced system responsiveness.
Possible Values:
- 0 : Default behavior.
- 1 : Prefer spinlocks for internal synchronization.
Example:
CUDA_PREFER_SPINLOCKS_ON_HOST=1
5.2.3.10. CUDA_AUTO_BOOST [[deprecated]]#
...
Possible Values:
- DEFAULT : Default behavior, equivalent toLAZY .
- LAZY : The loading of specific kernels is delayed until a CUDA function handle,CUfunc ,  is extracted using thecuModuleGetFunction() orcuKernelGetFunction() API calls. In this case, the data from the CUBIN is loaded when the first kernel in the CUBIN is loaded or when the first variable in the CUBIN is accessed.
...
- EAGER : Fully loads CUDA modules and kernels at program initialization. All kernels and data from a CUBIN, FATBIN, or PTX file are fully loaded upon the correspondingcuModuleLoad* andcuLibraryLoad* driver API call.
...
- EAGER : All data from a CUBIN, FATBIN, or PTX file are fully loaded upon the correspondingcuModuleLoad* andcuLibraryLoad* API call.
Example:
...
When this environment variable is set to 1, the driver pre-allocates the worst-case per-thread stack memory during the corresponding cuModuleLoad* call, based on the maximum requirement across all functions in that module. By default, the CUDA driver allocates per-thread stack memory on demand: when a kernel launch requires more stack than is currently available, the driver expands the allocation before execution.
Possible Values:
- 0 or unset: Stack memory is expanded on demand at kernel launch (default).
- 1 : Pre-allocate maximum stack memory at module load time.
Example:
CUDA_DEVICE_DEFAULT_STACK_SIZE_MAX=1
