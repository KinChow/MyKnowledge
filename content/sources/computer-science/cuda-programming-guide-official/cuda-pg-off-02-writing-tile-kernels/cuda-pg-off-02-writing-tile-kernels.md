...
attachments:
- filename: cuda-pg-off-02-writing-tile-kernels.html
  kind: document
  media_type: text/html
  role: original
  sha256: sha256:ff43cbedb5be0aad1d0a040fac9df657a21427b9a4cce61a6582caefb0ab2a83
confidentiality: public
domain: computer-science
extractor: trafilatura/2.2.0
id: cuda-pg-off-02-writing-tile-kernels
media_type: text/html
origin: external
raw_ref:
  path: archive/raw/ff43cbedb5be0aad1d0a040fac9df657a21427b9a4cce61a6582caefb0ab2a83.html
  sha256: sha256:ff43cbedb5be0aad1d0a040fac9df657a21427b9a4cce61a6582caefb0ab2a83
read_status: retrieved
retrieval:
  acquisition: fetch
  resolved_url: https://docs.nvidia.com/cuda/cuda-programming-guide/02-basics/writing-tile-kernels.html
  url: https://docs.nvidia.com/cuda/cuda-programming-guide/02-basics/writing-tile-kernels.html
schema_version: source/v1
snapshot_sha256: sha256:9acee9259d831278e8c7683919416d8bef4deb167510f48ddb726e8bb0d91d73
source_type: doc
vault_id: public
---
...
In CUDA Tile C++:
- __tile_global__ is the tile analog of__global__ and marks a tile kernel entry point
- __tile__ is the tile analog of__device__ and indicates a function that should be compiled for the GPU and be callable from other__tile__ or__tile_global__ functions.
...
- ct::zeros<Tile>() andct::ones<Tile>() - tiles filled with zeros or ones.
- ct::full<Tile>(val) - tile where every element has valueval .
- ct::iota<Tile>() -  tile containing(0, 1, ..., N-1) , whereN is the tile’s size.
...
- ct.zeros(shape, dtype) andct.ones(shape, dtype) - tiles filled with zeros or ones.
...
- Chunks of a fixed shape separated by a stride, so chunks may leave gaps between them or overlap. C++ spells this ct::strided_view ; Python passes atraversal_steps argument toArray.tiled_view .
- Chunks selected along one dimension by a tile of element indices, for the irregular access patterns introduced in Gather and Scatter. C++ spells this ct::gather_scatter_view ; Python usesct.load_advanced_indexing andct.store_advanced_indexing .
...
- ct::tensor_span - pairs a raw pointer with act::extents , giving the pointer multidimensional structure.
...
- C++ uses tiles of pointers passed to ct::load() /ct::store() , with masked variantsct::load_masked() andct::store_masked() that accept a boolean mask tile to handle tiles at array boundaries.
...
- Tile combined with tile: the result is a tile of the type with the greater precision or range. For example: 
  - int + float yieldsfloat
  - int16 + int32 yieldsint32
- Scalar combined with tile: when the scalar’s type is exactly representable in the tile’s element type (e.g., the integer literal 2 combined with anint tile, or2.0f combined with afloat tile), the operation proceeds in the tile’s element type. When the scalar would have to narrow to fit the tile’s element type (e.g., the literal2.5 combined with anint tile), the two languages differ:
...
- For memory operations such as ct::load ,ct::store , andct::partition_view loads/stores, place it on the expression-statement containing the call.
...
- Kernel-level hints are keyword arguments to the @ct.kernel(...) decorator. A compiled kernel object also has a.replace_hints(**hints) method that returns a new kernel with overridden hints; the new kernel has its own JIT cache, which makesreplace_hints the natural building block for autotuning loops.
- Per-call hints are keyword arguments on memory-operation call sites: ct.load /ct.store ,TiledView.load /TiledView.store , andct.gather /ct.scatter .
...
- Allowed values: 1 ,2 ,4 ,8 ,16 . Onsm_80 , only1 is applicable.
...
- Applies to: tile-space loads and stores ( ct::partition_view in C++;Array.tiled_view andct.load /ct.store in Python) as well as gather/scatter (ct::load /ct::store with pointer tiles in C++;ct.gather /ct.scatter in Python).
- Allowed values: any integer in the inclusive range [1, 10] , where1 indicates light DRAM traffic and10 indicates heavy traffic. Larger values typically cause the compiler to schedule a larger prefetch depth.
...
- Applies to: tile-space loads and stores only ( ct::partition_view in C++;Array.tiled_view andct.load /ct.store in Python). Gather and scatter operations do not accept this hint.
- Allowed values: true /false (C++) orTrue /False (Python). TMA is allowed by default; setting the hint tofalse /False instructs the compiler not to lower this particular load or store to TMA on hardware that supports it.
