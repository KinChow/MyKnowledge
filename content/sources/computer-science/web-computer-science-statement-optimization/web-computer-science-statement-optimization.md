...
attachments:
- filename: web-computer-science-statement-optimization.html
  kind: document
...
evidence_items:
- evidence_id: evidence-135cb2b83265
  position:
    end: 457
    start: 0
  ...
  selector:
    exact: 'LLVM’s Analysis and Transform Passes#
      ...
      LLVM provides. Optimizations are implemented as Passes that traverse some portion
      of a program to either collect information or transform the program.'
    prefix: ''
...
retrieval:
  acquisition: fetch
  resolved_url: https://llvm.org/docs/Passes.html
...
Dependence analysis framework, which is used to detect dependencies in memory accesses.
...
graph.  This graph can then be processed with the dot tool to
...
Reports various statistics for codes compiled for GPUs. This pass is documented separately.
...
Combine instructions to form fewer, simpler instructions. This pass does not modify the CFG. This pass is where algebraic simplification happens.
...
- Bitwise operators with constant operands are always grouped so that shifts are performed first, then or s, thenand s, thenxor s.
...
operations.
- add X, X is represented asmul X, 2 ⇒shl X, 1
...
-function-attrs pass and LLVM’s knowledge of
...
This pass aims to transform LLVM Modules into a normal form by reordering and renaming instructions while preserving the same semantics. The normalizer makes it easier to spot semantic differences while diffing two modules which have undergone two different passes.
...
LoopUnswitching, simpler. You can read more in the
loop terminology section for the LCSSA form.
...
Merges adjacent loops when it can prove the transformation preserves the program’s semantics. This pass is documented separately.
...
This pass implements a simple unroll and jam classical loop optimisation pass. It transforms a loop from:
for i.. i+= 1              for i.. i+= 4
  for j..                    for j..
    code(i, j)                 code(i, j)
                               code(i+1, j)
...
calls, or transforming sets of stores into memsets.
mergefunc: Merge Functions#
This pass looks for equivalent functions that are mergeable and folds them.
Total-ordering is introduced among the functions set: we define comparison that answers for every two functions which of them is greater. It allows to arrange functions into a binary tree.
...
In the implementation of this algorithm, constants are assigned rank = 0, function arguments are rank = 1, and other values are assigned ranks corresponding to the reverse post-order traversal of the current function (starting at 2), which effectively gives values in deep loops higher rank than values not in loops.
...
Verifies LLVM IR code. This is useful to run after an optimization which is undergoing testing. Note that llvm-as verifies its input before emitting bitcode, and also that malformed bitcode is likely to make LLVM crash. All language front-ends are therefore encouraged to verify their output before performing optimizing transformations.
