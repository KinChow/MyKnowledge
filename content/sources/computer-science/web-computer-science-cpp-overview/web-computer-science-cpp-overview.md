...
attachments:
- filename: web-computer-science-cpp-overview.html
  kind: document
...
retrieval:
  acquisition: fetch
  resolved_url: https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines
...
empty body –
loop variable –
loop variable type ???inline:
small functions –
in headers{} –
lambdas –
default member initializers –
class members –
factory functionspublic, private, and protected:
information hiding –
consistency –
static_assert:
...
T as a U through casts, unions, or varargs)delete or multiple nullptr, using a dangling reference).
...
where
"tag" is a string literal with the anchor name of the item where the Enforcement rule appears (e.g., for C.134 it is “rh-public”), the
...
f, base, and x.vector rather than std::vector.
...
const consistently (check if member functions modify their object; check if functions modify arguments passed by pointer or reference)
...
variant (in C++17)span (from the GSL)narrow or narrow_cast (from the GSL) where they are necessary
...
owner from the GSL.fopen, malloc, and strdup)
This is C++.
...
operator++ or operator-- function. Prefer using the prefix form instead. (Note: “User-defined non-defaulted” is intended to reduce noise. Review this enforcement if it’s still too noisy in practice.)
...
myX is in multi-threaded code,X object needs to be destroyed (e.g., because it releases a resource), and
...
void* as a parameter or return type.bool parameter.
...
owner<T>. Suggest use of standard-library resource handle or use of reset or explicitly To help avoid dereferencing nullptr errors.
...
constexpr functionsextern objects
...
operator-> or operator*) that is copyable but the function only calls any of: get().
...
&&. See F.18.const& (for lvalues),
add an overload that passes the parameter by std::moves it to its destination. Essentially this overloads a “will-move-from”; see F.18.int multiply(int, int); // just input ints, pass by value
...
4 * sizeof(void*).
Suggest using a reference to 2 * sizeof(void*). Suggest passing by value instead.
...
X&& parameters (where If the object is to be passed onward to other code and not directly used by this function, we want to make this function agnostic to the argument const-ness and rvalue-ness.
...
TP&& parameter (where TP is a template type parameter name) and does anything with it other than std::forwarding it exactly once on every static path, or A return value is self-documenting, whereas an & could be either in-out or out-only and is liable to be misused.
...
unique_ptr or shared_ptr.array<BigTrivial>), consider allocating it on the free store and return a handle (e.g., std::string, Assuming that Matrix has move operations (possibly by keeping its elements in a std::vector):
...
pair or A pointer (T*) can be a nullptr and a reference (T&) cannot, there is no valid “null reference”.
...
std::free(), etc. applied to a plain malloc(), etc. assigned to a plain To avoid the crashes and data corruption that can result from the use of such a dangling pointer.
...
fx and gx were of different types.
...
this or any class data member, don’t use [=] default capture
...
va_list, va_start, or va_arg.[[suppress("type")]].
...
struct Data { /*...*/ };
Data data{ /*...*/ };
} of a class or enumeration definition is not followed by a ;. The Readability. To make it clear that something is being hidden/abstracted. This is a useful convention.
struct Date {
    int d, m;
    Date(int i, Month m);
    // ... lots of functions ...
...
B uses another data member A, declare A concrete type is fundamentally simpler than a type in a class hierarchy: easier to design, easier to implement, easier to use, easier to reason about, smaller, and faster. You need a reason (use cases) for using a hierarchy.
class Point1 {
    int x, y;
    // ... operations ...
...
X()X(const X&)operator=(const X&)X(X&&)operator=(X&&)~X()
...
=delete any copy, move, or destructor function, define or Destructor rules:
Constructor rules:
Copy and move rules:
...
For example, a class with a (pointer, size) pair of members and a destructor that deletes the pointer could probably be converted to a vector.
...
make the copy and move operations protected or =deleted, and add a clone:
class CloneableBase {
public:
...
final_action.class Foo {   // bad; use the default destructor
public:
    // ...
...
gsl::owner), then they should be referenced in its destructor.
...
Also, the default for int would be better done as a default member initializer.
...
Ensures contract, try to see if it holds as a postcondition.
...
= without a default constructor== but not copyable
...
It is simple and efficient. If you want to optimize for rvalues, provide an overload that takes an && (see F.18).
class Foo {
public:
...
const T& which interfere with composability and putting objects in containers.
...
operator==() for which the argument types differ; same for other comparison operators: !=, <, <=, >, and >=.
...
hashes.
The standard C++ mechanism to construct an instance of a type is to call its constructor. As specified in guideline C.41: a constructor should create a fully initialized object. No additional initialization, such as by memcpy, should be required.
...
D does not override a virtual function or access a protected member in A class is more stable (less brittle) if it does not contain data. Interfaces should normally be composed entirely of public pure virtual functions and a default/empty virtual destructor.
class My_interface {
public:
...
override means exactly and only “this is a non-final overrider.”struct B {
    void f1(int);
...
Shape, the constructors get harder to write and maintain.Triangle? We might never use it.
...
Alternatively, and often better, keep data out of any class used as an interface.
...
static_cast for downcasts, including C-style casts that perform a Casting to a reference expresses that you intend to end up with a valid object, so the cast must succeed. dynamic_cast will then throw if it does not succeed.
std::string f(Base& b)
{
...
There are two kinds of enumerations, “plain” enums and class enums.
...
switch-statements where the cases cover most but not all enumerators of an enumeration.default.
...
Unique_pointer or Shared_pointer that is not moved, copied, reassigned or If your stack space is limited, it is OK to create a local const unique_ptr<BigObject> to store the object on the heap instead of the stack.
See I.2
...
shared_ptrs rely on use counting and the use count for a cyclic structure never goes to zero, so we need a mechanism to
...
Unique_pointer<T> parameter by lvalue reference and does not either assign to it or call reset() on it on at least one code path. Suggest taking a Using unique_ptr in this way both documents and enforces the function call’s reseating semantics.
...
Shared_pointer<T> parameter by lvalue reference and does not either assign to it or call shared_ptr<widget>& parameter to express that a function might reseat the shared pointer
...
auto, the name of the declared entity is in a fixed position in the declaration, increasing readability.auto p = v.begin();      // vector<DataRecord>::iterator
...
std::array) where the indexer is not a compile-time constant expression with a value between 0 and the upper bound of the array.
...
float->char and double->int. Here be dragons! We need data.)long->
...
std::variantstd::ignore = to ignore [[nodiscard]] values.void.Type(value). Use Type{value} instead which is not narrowing. (See ES.64.)
...
The named casts are:
const_castreinterpret_castmove(x) is an rvalue reference to forward<T>(x) is an rvalue or an lvalue reference to gsl::narrow_cast  // narrow_cast<T>(x) is static_cast<T>(x)gsl::narrow       // narrow<T>(x) is static_cast<T>(x) == x or it throws narrowing_errorclass B { /* ... */ };
class D { /* ... */ };
D* downcast(B* pb)
{
...
std::move(x) where return std::move(local_variable); and std::move(f()) on a function that returns by value.S&& parameter if there is no const S& overload to take care of lvalues.T&& where Direct resource management in application code is error-prone and tedious.
...
if-then-else chain.enum been covered? If not, is there a void use(int n)
{
    switch (n) {   // good
    case 0:
        // ...
...
goto. Better still flag all In a non-trivial loop body, it is easy to overlook a break or a continue.
...
This might yield too many false positives in some code bases; if so, flag only switches that handle most but not all cases
...
sizeof or a call to container .size() and the other is ptrdiff_t.
...
thread_local instead of static.static std::mutex.
...
For further information of how to use synchronization well to eliminate data races, please consult a good book about concurrency (see Carefully study the literature).
...
allowed to run again, and Local static variables are a common source of data races.
...
lock()/unlock()std::lock() or std::scoped_lock to acquire multiple mutexesthread as a scoped containergsl::joining_thread over std::threaddetach() a threadwait without a conditionlock_guards and unique_lockssynchronized_value<T> where possibletry_lock()new thread
...
future to return a value from a concurrent taskasync() to spawn concurrent tasks
...
volatile T local and data members; almost certainly you intended to use atomic<T> instead.
...
Here, vector and strings constructors might not be able to allocate sufficient memory for their elements, vectors constructor might not be able to copy the Things in its initializer list, and File_handle might not be able to open the required file.
...
operator delete, must be throw std::string(...) could throw but does not in practice.
...
concepts when used outside the definition of other enable_if that appear to simulate single-operation Ease of comprehension. Improved interoperability. Helps implementers and maintainers.
...
typename as a disambiguator outside using declarations.
...
x.size() == 0 when x.empty() or x.is_empty() is available. Emptiness works for more containers than size(), because some containers don’t know their size or are conceptually of unbounded size.
...
.cpp suffix for code files and .h for interface files if your project doesn’t already follow another convention
See NL.27
...
using namespace directives for transition, for foundation libraries (such as std), or within a local scope (only)
...
in header files and given the rules - users are required
...
The standard provides flexibility for compilers to implement
...
array or C arrays are less safe, and have no advantages over array and vector.
...
v.begin(),v.begin()+i the i can easily be checked against v.size()
...
TODO Notes:
memcmp and shipping them in the GSL.
...
char*free() on a The variety of uses of char* in current code is a major source of errors.
char arr[] = {'a', 'b', 'c'};
...
for (complex<double> z; cin >> z; )
    v.push_back(z);
??? performance ???
iostreams vs. the printf() family
...
This has to be weighed against iostreams advantages of extensibility to handle user-defined types, resilience against security violations,
...
return statements (and to throw exceptions).
...
Remember
Expects and The resulting number of files from placing each class in its own file are hard to manage and can slow down compilation. Individual classes are rarely a good logical unit of maintenance and distribution.
...
owner<T*>   // a owner is used to mark owning pointers in code that cannot be upgraded to use proper resource handles.
...
span<T>       // [p:p+n), constructor from {p, q} and {p, n}; span_p<T>     // {p, predicate} [p:q) where q is the first element for which predicate(*p) is true
...
zstring    // a czstring   // a const char* supposed to be a C-style string; that is, a zero-terminated sequence of Logically, those last two aliases are not needed, but we are not always logical, and they make the distinction between a pointer to one char and a pointer to a C-style string explicit.
...
unique_ptr<T>     // unique ownership: std::unique_ptr<T>shared_ptr<T>     // shared ownership: std::shared_ptr<T> (a counted pointer)stack_array<T>    // A stack-allocated array. The number of elements is determined at construction and fixed thereafter. The elements are mutable unless dyn_array<T>      // A container, non-growing dynamically allocated array. The number of elements is determined at construction and fixed thereafter.
The elements are mutable unless Expects(p) terminates the program unless p == true
...
post_initialize.operator new. create can assuage this problem, however; and the overloads can even be templated on the argument types.
...
Base or else be a hidden internal implementation detail.
...
[0:max).while-statement.[0:5) means the values 0, 1, 2, 3, and 4.
...
std::literals::*_literals)?const_iterators(size) vs. {initializers} vs. {Extent{size}}std::function)  vs. CRTP/static? YES Perhaps even vs. tag dispatch?std::bind, Stephen T. Lavavej criticizes it so much I’m starting to wonder if it is indeed going to fade away in future. Should lambdas be recommended instead?p = (s1 + s2).c_str();
...
shared_lock), never call mutex.lock and mutex.unlock directly (RAII)std::terminate in destructor if not joined or detached … is there a good reason to detach threads?) – ??? could support library provide a RAII wrapper for std::lock (or another deadlock avoidance algorithm?)condition_variable, always protect the condition by a mutex (atomic bool whose value is set outside of the mutex is wrong!), and use the same mutex for the condition variable itself.atomic_compare_exchange_strong with std::atomic<user-defined-struct> (differences in padding matter, while compare_exchange_weak in a loop converges to stable padding)shared_future objects are not thread-safe: two threads cannot wait on the same individual shared_ptr objects are not thread-safe: different threads can call non-const member functions on different shared_ptrs that refer to the same shared object, but one thread cannot call a non-const member function of a shared_ptr object while another thread accesses that same shared_ptr object (if you need that, consider atomic_shared_ptr instead)