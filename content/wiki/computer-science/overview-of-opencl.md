...
- claim: Khronos 的 OpenCL 页面说明，OpenCL 通过核心规范纳入已广泛部署的能力，并支持开发者依赖跨实现能力。
  claim_id: opencl-overview-cross-platform
  support: direct
  supporting_quotes:
...
- working-computer-science-overview-of-opencl
status: published
tags:
- opencl
- gpu
...
OpenCL 是一套跨平台并行编程开放标准，能让程序同时使用 CPU、GPU、FPGA 等异构处理器协同计算；它由平台、执行、内存、编程四层模型构成，开发者通过 host 程序 + kernel 程序 + 命令队列即可在设备上并行执行任务。

## 核心概念

- **四大模型**：平台模型（host + device + CU + PE）、执行模型（context/命令队列/命令状态/kernel/索引空间）、内存模型（内存区域/内存对象/SVM）、编程模型（平台层、runtime、compiler）。
- **命令队列**：与单个 device 关联，支持内核入队、内存、同步三类命令；阻塞/非阻塞；in-order/out-of-order。
- **命令六状态**：Queue → Submitted → Ready → Running → Ended → Complete。
- **内核分类**：OpenCL kernel、native kernel、built-in kernel。
- **索引空间与工作项**：NDRange（N=1/2/3），work-item 的全局/局部 ID，work-group。

## 工作机制

1. host 创建 context（设备集合、内核对象、程序对象、内存对象）与命令队列。
2. host 把 kernel（含参数与索引空间）入队到命令队列，命令经六个状态执行。
3. 设备在 CU/PE 上按 work-group 调度 work-item。
4. 内存通过读/写/填充、映射/解映射、拷贝三类命令在 host 与设备间传输；OpenCL 2.0 提供 SVM 共享虚拟内存。
5. 基础教程演示从获取平台/设备、创建 context/队列/缓冲/程序/内核，到执行与读取结果的完整流程。

## 示例或代码

vector_add 完整示例（见 ## 详细章节）：并行加法 kernel + host 端 16 步流程，以及 Linux/Windows 编译命令。

## 常见误区

- 误以为 work-item 之间一定并行执行（实际执行模型依赖硬件）。
- 误以为入队命令立即执行（实际可能是异步执行）。
- 误以为 local/private 内存与全局内存在物理上必然不同。
- 忽略 NDRange 偏移 F 的默认值 0。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| opencl-overview-cross-platform | khronos-opencl-overview-v2 | OpenCL 3.1 核心规范纳入 field-proven 能力（含 SPIR-V） |
...
- [[opencl]] —— OpenCL 架构与模型详解。
- [[cl-mem]] —— 内存对象对比。
- [[limitations-of-local-work-size-and-global-work-size]] —— work size 限制。
