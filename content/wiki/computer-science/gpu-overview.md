...
- working-computer-science-gpu-overview
status: published
tags:
- gpu
- parallel-computing
...
GPU（图形处理器）是专为**并行处理**和**快速渲染图像、视频和图形**设计的微处理器芯片，拥有成百上千个更小更高效的核心，可同时处理大量相似任务；它从固定管线演进到可编程/统一着色器，并借 CUDA/OpenCL 等平台成为大规模并行通用计算（GPGPU）的主力，与 CPU 互补。

## 核心概念

- **GPU / GPGPU**：图形处理器（Graphics Processing Unit）；GPGPU 指用 GPU 做大规模、高度并行的通用计算任务。
- **图形渲染管线**：顶点处理 → 三角形处理 → 光栅化 → 片段处理 → 帧缓冲操作。
- **统一着色器架构**：流处理器核心通用化，可动态分配执行顶点、像素、几何、曲面细分等任务。
- **GPU 架构组件**：流处理器（NVIDIA SM / AMD CU）、纹理映射单元（TMU）、光栅操作单元（ROP）、显存（VRAM）、高速缓存、总线接口（PCIe）、显示输出接口、专用硬件加速单元（视频编解码引擎、光追核心、张量核心）。

## 工作机制

- **渲染管线五阶段**：顶点着色器做坐标变换与光照 → 组装三角形 → 光栅化为片段 → 像素着色器做颜色计算（纹理、光照、透明度混合）→ 帧缓冲操作（深度测试、混合）输出像素。每个顶点/片段独立处理，可大规模并行。
- **并行模型**：GPU 是 SIMT（本质为特殊的 SIMD），大量 CUDA 核心在同一时间执行同样指令，适合计算密集、易于并行的程序。
- **与 CPU 分工**：CPU 负责复杂逻辑调度与整体程序控制（延迟敏感），GPU 负责计算密集型"重活"（吞吐量优先），两者互补。

## 示例或代码

- **CPU vs GPU 关键对比**：CPU 核心少而复杂（强大的分支预测、乱序执行），GPU 核心多而简化；CPU 用低延迟系统内存（DDR），GPU 用高带宽专用显存（GDDR/HBM）；CPU 编程模型 MIMD/多线程，GPU 为 SIMT/SIMD/GPGPU（CUDA、OpenCL）。
- **GPU 适合的程序**：计算密集型（大部分运行时间消耗在寄存器运算上，从寄存器读写数据几乎没有时延）；易于并行（大量数据可同时执行同样指令）。

## 常见误区

- **“GPU 核心和 CPU 核心一样复杂”**：GPU 核心更小、更简化，专注基本浮点/整数运算，靠数量取胜。
- **“GPU 适合所有计算”**：GPU 适合计算密集、易于并行的程序；延迟敏感、逻辑复杂、需要分支的任务更适合 CPU。
- **“CPU 和 GPU 是竞争关系”**：两者在现代系统中互补——CPU 做复杂逻辑调度，GPU 做大规模并行计算。

## 证据映射

| Claim | 来源 | 要点 |
| --- | --- | --- |
| gpu-overview-audit-1 | nvidia-cuda-programming-guide-v2 | CUDA 是 NVIDIA 开发的并行计算平台和编程模型，借助 GPU 大幅提升计算性能 |
...
- [[davinci]] —— 华为达芬奇 AI 计算架构。
- [[soc]] —— GPU 作为 SoC 组成部分。
- [[cpu-and-memory]] —— CPU 与内存机制对比。
- [[mali-gpu-overview]] —— ARM Mali GPU 概览。
- [[opencl]] —— 异构计算编程模型。
