...
evidence:
- claim: '- RISC uses a small set of simple, fixed-length instructions and follows
    a load/store approach, enabling efficient and fast execution.'
  claim_id: types-audit-1
  support: direct
  supporting_quotes:
...
- working-computer-science-types
status: published
tags:
- architecture
- cpu
...
指令集类型可大致分为 **CISC（复杂指令集）、RISC（精简指令集）与 VLIW（超长指令字）** 三类：CISC 用大量复杂、长度不定的指令简化编程、提高代码密度；RISC 用精简、定长的指令配合 load/store 与流水线换取高效率与低功耗；VLIW 则把指令级并行（ILP）的识别交给编译器，在一个长指令字内并行发射多条操作。

## 核心概念

- **CISC（Complex Instruction Set Computer）**：每条指令可执行若干低端操作（读存储、存存储、计算），指令多样、寻址模式丰富、长度可变、硬件实现复杂。代表：x86、VAX、PDP-11、Motorola 68000 等。
- **RISC（Reduced Instruction Set Computer）**：对指令数目与寻址方式做精简，指令数量少、格式简单统一、定长，大量使用寄存器并配合 load/store 访存。代表：ARM、MIPS、RISC-V、PowerPC、SPARC 等。
- **VLIW（Very Long Instruction Word）**：把多个相互独立的简单指令组合成一个长指令字，在一个指令周期内并行执行多条指令，并行性由编译器在编译期确定。代表：DSP、GPU（如 ATI Radeon）等。

## 工作机制

- **CISC 的执行路径**：指令可以直接访问内存（可访存指令不限制），通过丰富的寻址模式减少访存次数、提高代码密度；但解码复杂，指令执行时间相差较大，硬件控制逻辑与解码器更复杂。
- **RISC 的执行路径**：采用 load/store 访存模型，绝大多数操作在寄存器中完成；定长、格式统一的指令简化了解码，配合流水线与并行处理，大多数指令在一个或少数几个时钟周期内完成，功耗更低、更易扩展与维护。
- **VLIW 的执行路径**：指令字固定长度，硬件无需动态调度逻辑；编译器负责识别并行指令并调度，通过并行发射多条指令提高吞吐率，但代码密度与可移植性受限。

## 示例或代码

常见微处理器架构归属：

- **CISC**：CDC 6600、System/360、VAX、PDP-11、Motorola 68000 家族、x86、AMD Opteron。
- **RISC**：DEC Alpha、ARC、ARM、AVR、MIPS、PA-RISC、Power ISA（含 PowerPC、PowerXCell）、RISC-V、SPARC。
- **VLIW**：数字信号处理器（DSP）、图形处理单元（GPU，如 ATI Radeon 系列）及部分嵌入式系统。

CISC 与 RISC 对照：

| 对比类别 | CISC | RISC |
| --- | --- | --- |
| 代表架构 | x86 | ARM、RISC-V、PowerPC、MIPS |
| 指令系统 | 复杂、庞大 | 简单、精简 |
| 指令条数 | 一般大于 200 条 | 一般小于 100 条 |
| 指令字长 | 不固定 | 定长 |
| 可访存指令 | 不限制 | load/store 访问 |
| 指令执行时间 | 相差较大 | 基本都在一个周期内完成 |
| 通用寄存器数目 | 较少、多使用内存 | 较多、绝大多数操作在寄存器中完成 |

## 常见误区

- **“RISC 一定比 CISC 快”**：RISC 指令执行更快、能效更高，但 CISC 代码密度高、内存访问优化好、编程简便，两者在性能与能效间各有取舍，不能一概而论。
- **“VLIW 的并行由硬件动态调度”**：恰恰相反，VLIW 的并行性由编译器在编译期识别并调度，硬件无需复杂动态调度逻辑。
- **“RISC 每条指令都在一个时钟周期内完成”**：原文表述是“基本都在一个周期内完成”，“基本”意味着并非绝对，复杂指令仍可能多周期执行。
- **“CISC 已完全被淘汰”**：x86 等 CISC 架构仍是主流桌面/服务器处理器，其指令密度与内存访问优化在现实中仍有价值。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| types-audit-1 | risc-cisc | RISC 用精简、定长的简单指令集并遵循 load/store 方式，执行高效快速 |
...
- [[architecture-and-organization]] —— 计算机体系结构总体框架，指令集是其核心组成。
- [[cpu-and-memory]] —— 指令执行与访存路径的关系。
- [[cpu-pipeline-and-hazards]] —— RISC 流水线与并行执行的基础。
- [[compiler-vectorization]] —— 编译器如何利用指令级并行（与 VLIW 编译器调度相关）。
