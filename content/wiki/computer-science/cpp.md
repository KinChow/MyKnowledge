...
evidence:
- claim: 课程第5章包含指针、指针与数组，以及 C/C++ 风格的内存分配。
  claim_id: cpp-claim-1
  support: direct
  supporting_quotes:
...
- claim: 课程第6章包含函数、函数参数、引用、return 语句和 inline 函数。
  claim_id: cpp-claim-2
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-7b102c6d4c89
    source_id: web-computer-science-cpp
- claim: 课程第11章包含动态内存示例，以及 Hard copy、Soft copy 和 Smart pointers。
  claim_id: cpp-claim-3
  support: direct
  supporting_quotes:
  - evidence_id: evidence-a570371fba3c
    exact: '## [Chapter 11: Dynamic Memory Management in Classes](week11/README.md)

      1. Some default operations 1. An example with dynamic memory
  ...
  - evidence_id: evidence-a570371fba3c
    source_id: web-computer-science-cpp
- claim: 课程第13章包含类模板、非类型模板参数、类模板特化和 std 类。
  claim_id: cpp-claim-4
  support: direct
  supporting_quotes:
...
C++ 是几乎完美兼容 C 的面向对象语言，本页汇集了 C++ 核心知识的问答式总结：从指针/引用、static/const、内存管理（栈/堆/new/malloc）、类与对象（构造/析构/继承/多态）、移动语义、STL 容器，到模板、智能指针/RAII、lambda 与编译链接等，覆盖 C++ 语言关键机制与常见陷阱。

## 核心概念

- **指针与引用**：引用是已存在变量的别名，必须在声明时初始化、不能被重新绑定、不能为 nullptr；指针是存储地址的变量，可重新指向、可为 nullptr、可做地址运算。汇编层面引用本质上是指针的一种特殊表示，但编译器通常将其优化为直接使用目标变量。
- **static/const/extern/mutable**：static 控制生命周期、作用域与链接性（静态局部/全局/成员）；const 声明只读数据并做编译期检查；extern 声明变量/函数定义在其他处；mutable 允许在 const 上下文中修改成员。
- **内存管理**：栈（系统管理、地址向下增长）、堆（用户 new/delete 申请释放、地址向上增长）、全局/静态区；malloc/free 用内存池、分区、对齐、头部信息与合并策略管理堆内存。
- **类与对象**：构造函数（基类→成员→派生类）、析构函数、拷贝/移动构造、三/五/零法则、虚函数、抽象基类、组合与继承。
- **移动语义**：左值/右值、`std::move`/`std::forward`、移动构造与移动赋值。
- **STL**：vector（连续存储、两倍扩容）、list（链表）、map/set（红黑树）、unordered_map（哈希、冲突与扩容）、迭代器、allocator。
- **模板与 lambda**：编译期类型参数替换与实例化；lambda 匿名可调用对象。
- **RAII/智能指针**：shared_ptr/unique_ptr/weak_ptr 自动管理资源。

## 工作机制

- **引用与指针传参**：值传递、指针传递（传地址）、引用传递（创建别名）；引用避免拷贝开销且函数内修改直接反映到原始数据，指针需考虑生命周期/所有权/空指针检查。
- **new/delete 与 malloc/free**：`new` 先分配空间再调构造函数，`delete` 先调析构再释放；`malloc/free` 基于内存池、分区、对齐、头部信息、合并策略；`new` 调用构造、`malloc` 不调用。`delete[]` 配合 `new[]` 才能正确调用所有析构。
- **构造函数执行顺序**：基类构造函数（按继承列表顺序）→ 成员变量初始化（按声明顺序）→ 派生类构造函数体；成员初始化列表直接构造、避免临时对象，通常更快。
- **多态与虚函数**：通过基类指针释放派生类时基类析构须为虚函数，否则只调基类析构导致内存泄漏；构造函数/析构函数中调用虚函数会未实现多态。
- **STL vector**：连续内存，扩容通常按两倍（也有 1.5 倍实现）重新分配并搬移元素；删除元素时迭代器会失效；释放空间需依赖析构与 allocator。
- **模板实例化**：编译器在模板被实例化时按类型参数生成具体版本，模板代码通常在头文件中定义以便多个源文件包含。
- **编译链接**：C 语言编译链接过程（源文件 → 编译 → 目标文件 → 链接 → 可执行文件）；hello world 从程序开始到打印到屏幕经历预编译/编译/汇编/链接/加载执行。
...
```cpp
std::string str = std::to_string(123);  // int -> string
...
```cpp
const int *ptr;            // 指向常量的指针：不能通过它修改数据
int *const ptr = &value;   // 常量指针：地址不能改，数据可改
const int *const p = &v;   // 指向常量数据的常指针：两者都不可改
```

手写 UniquePtr（移动语义 + RAII）：
...
- **把引用当作可重新绑定的指针**：引用一旦初始化就始终指向该变量，不能被重新赋值指向另一个变量；引用不能被初始化为 nullptr。
- **以为 new/delete 与 malloc/free 等价**：`new` 调用构造函数、`delete` 调用析构函数；`malloc/free` 不调用；`malloc` 申请的空间不能用 `delete` 释放（反之亦然）。
- **用 `delete` 释放 `new[]` 的数组**：delete 只删除第一个元素，必须用 `delete[]` 才能调用所有析构函数。
- **浅拷贝共享内存导致双重释放**：含指针成员的类使用默认拷贝会共享同一块内存，析构时可能未定义行为，需要深拷贝（三/五法则）或智能指针。
- **构造函数/析构函数中调用虚函数**：不会产生多态行为；基类一次只构造一个完整对象。
- **基类析构函数不是虚函数**：通过基类指针释放派生类时不会调用派生类析构函数，造成内存泄漏。
- **以为 const 变量运行期不可改**：const 是编译期检查，用 const_cast 或 C 风格强转仍可修改；对全局常量强转赋值是未定义行为。
- **vector 扩容/删除误区**：vector 扩容按倍数增长（如两倍），删除中间元素会移动后续元素并使迭代器失效；删除元素不一定立即释放空间。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| cpp-claim-1 | web-computer-science-cpp | 课程第5章：指针、指针与数组、C/C++ 风格内存分配 |
| cpp-claim-2 | web-computer-science-cpp | 课程第6章：函数、函数参数、引用、return 语句、inline 函数 |
| cpp-claim-3 | web-computer-science-cpp | 课程第11章：动态内存示例、Hard copy、Soft copy、Smart pointers |
| cpp-claim-4 | web-computer-science-cpp | 课程第13章：类模板、非类型模板参数、类模板特化、std 类 |
...
- [[cpp-overview]]：C++ 语言概述（特性、演化、学习路径）
- [[cpp-class-and-raii]]：对象生命周期、RAII 与智能指针
- [[cpp-concurrent-programming-and-cuda]]：C++ 并发编程与 CUDA
- [[cpp-vs-c]]：C++ 与 C 对比
- [[cpp-vs-java]]：C++ 与 Java 对比
- [[cpp-vs-python]]：C++ 与 Python 对比
- [[cpp-keywords]]：C++ 关键字列表
