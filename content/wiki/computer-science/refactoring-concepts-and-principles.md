...
- claim: 'Refactoring
  ...
  claim_id: refactoring-concepts-and-principles-audit-1
  support: direct
  supporting_quotes:
...
id: refactoring-concepts-and-principles
kind: knowledge
...
- working-computer-science-refactoring-concepts-and-principles
status: published
tags:
- refactoring
- software-design
...
重构是在不改变代码外在行为的前提下改进内部结构的系统过程：它能化混乱为整洁代码与简单设计，改进软件设计、让代码更易理解、帮助找到 bug、提高编程速度并偿还技术债务。重构应随时随地进行，但不应为重构而重构。

## 核心概念

- **重构（名词）**：对软件内部结构的一种调整，目的是在不改变可观察行为的前提下，提高可理解性、降低修改成本。
- **重构（动词）**：使用一系列重构手法，在不改变可观察行为的前提下调整其结构。
- **坏味道是起点**：重构工作流从"识别坏味道"开始，再"采用手法"。
- **可信度分层**：L0 功能正确 → L1 无漏洞风险 → L2 代码整洁 → L3 架构敏捷 → L4 持续演进。
- **重构分类**：架构（框架）级、模块级、函数级。

## 工作机制

- **经典工作流**：测试保护 → 识别坏味道 → 采用手法 → 小步前进。
- **实际项目流程**：深入业务（挖掘问题/深入代码）→ 重构设计（领域建模/重构规则/兼容方案）→ 重构实施（工程基线/代码重构，分离变化、消除重复）→ 持续演进（模型演进/人员赋能）。
- **时机**：添加功能时、修补错误时、复审代码时；代码太乱、临近 deadline、无更好思路等情形不适合当场重构，应推迟但不可忽略。
- **原则**：添加新功能和重构一次只做一件事。

## 示例或代码

**三次法则**：事不过三，三则重构。第一次做某件事只管去做；第二次做类似的事会产生反感，但还是做了；第三次再做类似的事情，就应该重构。

**一次只做一件事**：添加新功能时不修改既有代码，只管添加功能；重构时不能添加功能，只管改进程序结构。

## 常见误区

- **为了重构而重构**：重构应帮助你更好地完成要做的事，而不是为了重构本身。
- **deadline 前做"最后一分钟的修改"**：应推迟重构，但不可忽略。
- **重构与加功能混在一起**：应一次只做一件事。
...
| claim_id | 来源 | 要点 |
| --- | --- | --- |
| refactoring-concepts-and-principles-audit-1 | web-computer-science-refactoring-techniques | 重构是系统化过程，化混乱为整洁代码与简单设计；脏代码源于经验不足 × 紧期限/管理不善/走捷径；整洁代码易读、易理解、易维护 |
...
- [[bad-smells-in-code]] —— 重构的第一步：识别坏味道。
- [[refactoring-techniques]] —— 具体的重构手法清单。
- [[four-rules-of-simple-design]] —— 简单设计四原则。
- [[solid]] —— 面向对象设计原则。
- [[orthogonal-design-principles]] —— 正交设计四原则，与重构的方向高度相关。
