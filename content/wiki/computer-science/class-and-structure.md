...
evidence:
- claim: 当 public 位于类成员列表前面时，这些成员可从任何函数访问，并持续适用于到下一个访问指示符或类末尾的所有成员。
  claim_id: class-and-structure-claim-1
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-fb2b18d5f22e
    source_id: web-computer-science-class-and-structure
- claim: 当 public 位于基类名称前面时，基类的公共成员和受保护成员在派生类中分别保持为公共成员和受保护成员。
  claim_id: class-and-structure-claim-2
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-8c85a56b407c
    source_id: web-computer-science-class-and-structure
- claim: 类中成员的默认访问权限是私有的，结构或联合中成员的默认访问权限是公共的。
  claim_id: class-and-structure-claim-3
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-5cd118805b9e
    source_id: web-computer-science-class-and-structure
- claim: 基类的默认访问对于类是私有的，对于结构是公共的，并且联合不能具有基类。
  claim_id: class-and-structure-claim-4
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-776668d876d8
    source_id: web-computer-science-class-and-structure
id: class-and-structure
kind: knowledge
publication_scope: public
related: []
schema_version: wiki/v1
sources:
- web-computer-science-class-and-structure
- working-computer-science-class-and-structure
status: published
tags:
- cpp
- oop
...
在 C++ 中，`struct`、`class` 与 `union` 都是用户自定义类型：**`class` 与 `struct` 唯一的本质区别是成员的默认访问权限（类默认 `private`，结构默认 `public`）**；`union` 则一次只活跃一个成员。继承时基类的默认访问同样对类为 `private`、对结构为 `public`，且联合不能有基类。

## 核心概念

- **三者的对比**：`struct`（默认公共、无使用约束）、`class`（默认私有、无使用约束）、`union`（默认公共、一次只有一个成员）。
- **成员访问控制**：`private`（仅成员函数与友元）、`protected`（成员函数、友元及派生类）、`public`（任意函数）。访问控制有助于阻止不适当地使用对象，但显式强制转换会丢失此保护。
- **派生类中的访问控制**：由“派生类是否用 `public` 声明基类”与“基类成员的访问权限”两个因素共同决定。
- **虚函数机制**：虚函数应在派生类中重定义，用基类指针/引用调用对象时执行派生类版本；纯虚函数则强制派生类实现（抽象类不可实例化）。

## 工作机制

- **成员默认访问**：类中成员的默认访问是私有的；结构或联合中成员的默认访问是公共的。访问指示符（`public:`/`protected:`/`private:`）之后的成员保持该访问级别，直到下一个访问指示符或类定义末尾。
- **继承访问推导**：

| 基类成员 | 私有派生 | 受保护派生 | 公共派生 |
| --- | --- | --- | --- |
| `private` | 始终无法通过任何派生访问 | 同左 | 同左 |
| `protected` | 派生类中为 `private` | 派生类中为 `protected` | 派生类中为 `protected` |
| `public` | 派生类中为 `private` | 派生类中为 `protected` | 派生类中为 `public` |

- **虚函数分派**：根据对象的基本类型（动态类型）解析对虚函数的调用；根据指针/引用的类型解析对非虚函数的调用。构造函数中调用虚函数只执行基类版本（派生部分未构造）；析构函数中调用虚函数可能引起资源二次释放等未定义行为。

## 示例或代码

```c++
struct S { int x; };      // 默认 public：x 可从任意函数访问
class C { int x; };       // 默认 private：x 只能由成员函数/友元访问

class Base { public: virtual void f(); };  // 虚函数，可在派生类中重定义
class Derived : public Base {              // public 派生：Base 的公共/受保护成员保持原级别
  public: void f() override;               // 显式重写基类虚函数
};

class Abstract { public: virtual void g() = 0; };  // 纯虚函数 → 抽象类，不能实例化
```
...
- **“`struct` 不能有方法或构造函数”**：C++ 中 `struct` 与 `class` 功能等价，只是默认访问不同；`struct` 同样可以有成员函数、构造函数、继承等。
- **“派生类能访问基类的 `private` 成员”**：基类 `private` 成员始终无法通过任何派生访问，这与派生方式无关。
- **“联合可以有基类”**：不能。联合不能具有基类。
- **“在构造函数中调用虚函数会执行派生类版本”**：不会。构造过程中对象动态类型尚未完全建立，只会执行基类版本；析构函数中调用虚函数也可能导致未定义行为，通常应避免。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| class-and-structure-claim-1 | web-computer-science-class-and-structure | public 在成员列表前时，成员可从任何函数访问，持续到下一指示符或类末尾 |
| class-and-structure-claim-2 | web-computer-science-class-and-structure | public 在基类名称前时，基类公共/受保护成员在派生类中保持原级别 |
| class-and-structure-claim-3 | web-computer-science-class-and-structure | 类成员默认私有，结构/联合成员默认公共 |
| class-and-structure-claim-4 | web-computer-science-class-and-structure | 基类默认访问对类私有、对结构公共，联合不能有基类 |

## 待验证项

- 构造函数/析构函数/移动/拷贝等成员函数的“执行顺序与内部行为”小节在原文中仅有标题无正文（如“构造函数的执行顺序？析构函数的执行顺序？”），待后续补充验证。

## 关联知识

- [[cpp]] —— C++ 语言整体知识。
- [[cpp-class-and-raii]] —— 类与 RAII 资源管理。
- [[conversion]] —— C++ 类型转换（含指向成员指针、虚继承下的转换）。
- [[cpp-vs-c]] —— C 与 C++ 在结构体/类语义上的差异。
...
- https://learn.microsoft.com/zh-cn/cpp/cpp/class-cpp?view=msvc-170
- https://learn.microsoft.com/zh-cn/cpp/cpp/struct-cpp?view=msvc-170
- https://learn.microsoft.com/zh-cn/cpp/cpp/public-cpp?view=msvc-170
