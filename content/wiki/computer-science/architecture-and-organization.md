...
evidence:
- claim: ARM 架构自问世以来已定义 9 个主要版本（1-9），前 3 个已废弃。
  claim_id: arm-nine-versions
  support: direct
  supporting_quotes:
...
- claim: AArch64 是 64 位执行状态：地址存于 64 位寄存器，指令可用 64 位寄存器处理，支持 A64 指令集。
  claim_id: aarch64-exec-state
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-3dc4401ba81e
    source_id: arm-a-profile-architectures
- claim: ARM 架构定义四个异常级别 EL0-EL3，EL3 最高、EL0 最低。
  claim_id: exception-levels
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-f20a3948ce05
    source_id: arm-d1-exception-levels
- claim: AArch64 有 31 个通用寄存器 R0-R30，每个可按 X0-X30（64 位）或 W0-W30（32 位）访问。
  claim_id: aarch64-registers
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-8c92e4905685
    source_id: arm-ddi0487-aarch64-registers
- claim: Cortex-A76 解码宽度提升到每周期 4 条指令（对比 A75 的 3 条）。
  claim_id: cortex-a76-decode
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-0b9e4d4f5b0c
    source_id: cortex-a76-deep-dive
- claim: Cortex-A76 保持 64KB 4 路组相联 L1 cache 与 256-512KB 私有 L2。
  claim_id: cortex-a76-cache
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-620cb82ae63b
    source_id: cortex-a76-deep-dive
- claim: Cortex-A76 配有两个 SIMD NEON 执行流水线（仅一个可处理浮点除法与乘加）。
  claim_id: cortex-a76-neon
  support: direct
  supporting_quotes:
  ...
  - evidence_id: evidence-5649ac82db63
    source_id: cortex-a76-deep-dive
- claim: ARM 内存模型用 Shareability 内存属性表示硬件须在多个观察者之间保证内存一致性的程度。
...
- claim: ARM 架构定义了应用层可见的分层存储系统（多级 cache），但存储系统细节是 IMPLEMENTATION DEFINED。
  claim_id: multi-core-cache
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-0881edb1b41c
    source_id: arm-b27-caches
id: architecture-and-organization
kind: knowledge
publication_scope: public
related: []
sources:
- arm-a-profile-architectures
- arm-d1-exception-levels
- arm-ddi0487-aarch64-registers
- arm-ddi0487-memory-model
- arm-b27-caches
- cortex-a76-deep-dive
status: published
tags:
- arm
- architecture
- cpu
title: ARM 体系结构与现代处理器设计
...
ARM 架构（ARMv1-v9，前 3 版已废弃）定义了 AArch64/AArch32 两种执行状态与 EL0-EL3 四个异常级别，构成编程模型基础；现代 ARM 处理器核（如 Cortex-A76）在此基础上通过宽解码、多流水线（含 SIMD NEON）与分层 cache 实现高性能。ARM 的 A/R/M 三种 profile 分别面向应用、实时与微控制器场景。

## 核心概念

- **架构版本与执行状态**：ARM 架构已定义 9 个主要版本，AArch64（64 位，A64 指令集）与 AArch32（32 位）是两种执行状态。
- **异常级别（Exception Level）**：EL0-EL3 四级，EL3 最高、EL0 最低，构成特权/非特权执行模型（见详细章节「ARM处理器模式」）。
- **A/R/M 三种 profile**：A（Application）面向高性能、支持 MMU 与 A64/A32/T32 指令集；R（Real-time）面向实时、支持 MPU 与 A32/T32；M（Microcontroller）面向低功耗低成本、支持 T32 变种。
- **AArch64 寄存器**：31 个通用寄存器 R0-R30（X0-X30/W0-W30 视图）+ SP/PC 等特殊寄存器。
- **Cortex-A76 微架构**：4 指令/周期解码、双 SIMD NEON 流水线、64KB 4 路 L1 + 256-512KB 私有 L2。

## 工作机制

现代 ARM 处理器核的指令执行链路（以 Cortex-A76 为参照）：

1. **取指与解码**：宽解码（每周期 4 条指令/8 条 16 位）将指令译为内部操作。
2. **寄存器重命名与调度**：通过 ROB（Re-order Buffer）支持乱序执行，消除数据冒险（见详细章节「寄存器重命名」）。
3. **并行执行**：多执行单元（整数/分支/负载存储）+ 双 SIMD NEON 流水线。
4. **存储系统**：分层 cache（L1/L2）+ MMU 地址翻译 + 内存一致性协议（见详细章节「cache」「MMU」）。

## 示例或代码

AArch64 寄存器访问示例（体现 R0-R30/X/W 视图）：

```assembly
// AArch64 通用寄存器：X 为 64 位视图，W 为低 32 位视图
MOV X0, #100      // 64 位寄存器
MOV W1, #200      // 32 位视图
```

异常级别与模式：`EL0`（用户态）→ `EL1`（内核态）→ `EL2`（虚拟机监视器）→ `EL3`（固件/Secure Monitor），异常通过异常向量表路由（见详细章节「异常向量表」）。

## 常见误区

- **AArch64/AArch32 是执行状态不是架构版本**：架构版本（ARMv8/v9）与执行状态（64/32 位）是两个维度。
- **`device` 状态 ≠ 系统就绪**（见 adb wiki）：异常级别 EL0-EL3 决定特权，不是处理器"模式"的传统含义。
- **Cortex-A76 的 NEON 两个流水线能力不对称**：只有其中一个能处理浮点除法与乘加。
- **cache 层级不等于一致性**：cache 层次（L1/L2）与内存一致性（Coherence）是不同问题（见详细章节）。

## 证据映射

| Claim | 来源 | 要点 |
| --- | --- | --- |
| arm-nine-versions / aarch64-exec-state | arm-a-profile-architectures | 9 个版本、AArch64 执行状态 |
| exception-levels | arm-d1-exception-levels | EL0-EL3 四级 |
| aarch64-registers | arm-ddi0487-aarch64-registers | R0-R30/X/W 视图 |
| memory-model | arm-ddi0487-memory-model | Shareability 属性 |
| cortex-a76-decode / -cache / -neon | cortex-a76-deep-dive | 解码宽度/cache/NEON 流水线 |
...
- [[isa-and-microarchitecture]] —— ISA 与微架构的层次拆分（本文 ARM 部分的扩展）。
- [[compilers-on-linux]] —— ARM 交叉工具链与 AArch64 目标。
- [[cpu-pipeline-and-hazards]] —— 流水线冒险与乱序执行（ROB 相关）。
...
ARM 架构自问世以来持续演进（依据已锚定的 ARM 官方手册 arm-a-profile-architectures）：

- **九个主要版本**：已定义 9 个主要架构版本（编号 1-9），其中前 3 个（ARMv1-ARMv3）已废弃。
- **执行状态维度**：AArch64（64 位执行状态，A64 指令集，地址存于 64 位寄存器）与 AArch32（32 位）是两种执行状态，与架构版本编号正交。
- **演进主线**：从早期 32 位 RISC 指令集，到 ARMv7 的 A/R/M profile 分化（A=应用、R=实时、M=微控制器），再到 ARMv8 引入 AArch64/A64，ARMv9 延续 AArch64 主线。


###### 路标

按 ARM 官方架构手册（arm-a-profile-architectures）定义的 9 个版本构成演进路标：

| 版本代 | 说明 |
| --- | --- |
| ARMv1-v3 | 已废弃（前 3 个主要版本） |
| ARMv4-v6 | 早期 32 位架构 |
| ARMv7 | A/R/M profile 分化：A（应用/MMU/A64·A32·T32）、R（实时/MPU/A32·T32）、M（微控制器/T32 变种） |
| ARMv8 | 引入 AArch64 执行状态（64 位，A64 指令集） |
| ARMv9 | 当前主线，延续 AArch64 并增强安全特性 |
...
-   指令执行（Instruction Execution）
  - 乱序执行指令
  -   指令结果、需要更新的寄存器或Memory地址等信息写入ROB(Re-Order Buffer)
...
多核处理器的存储系统由 ARM 架构定义应用级接口：分层 cache（多级 cache）加上一致性协议（见「证据映射」multi-core-cache）。

- **分层 cache 层次**：ARM 架构定义分层的存储系统，含多级 cache（L1/L2/L3），供应用层访问；但存储系统的具体实现细节（cache 大小、微架构布局）是 **IMPLEMENTATION DEFINED**（实现定义，随核而异）。
- **多核一致性**：多核间 cache 一致性依赖内存模型的 ordering/coherency 要求（见「证据映射」memory-model——Shareability 属性表示硬件须在多个观察者之间保证内存一致性的程度）。
- **DSU 集群**：现代 ARM 多核（如 Cortex-A76 族）通过 DynamIQ Shared Unit（DSU）共享 L3 cache 并管理核心一致性；片上互连（AMBA CCI/CMN）连接核心与 L3/内存控制器。
- **与单核的关系**：多核设计在单核微架构（宽解码、多执行流水线、分层 cache——见 Cortex-A76 深潜）之上叠加一致性协议与共享存储层次。
