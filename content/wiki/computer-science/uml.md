...
UML（统一建模语言）是用于**指定、可视化、构建和文档化软件系统**的标准图形语言：它不是编程语言，但工具可以用 UML 图生成各语言代码；它通过 **4+1 视图**（设计、实现、过程、部署、用例）与结构图/行为图两大类对复杂系统建模，是面向对象分析与设计的后续者。

## 核心概念

- **定义与目标**：UML 是制作软件蓝图的图形语言，目标是定义一种简单的建模机制，用于对复杂环境中的系统建模。
- **特点**：面向对象、可视化表达能力强、独立于开发过程、独立于程序设计语言、易于掌握使用。
- **4+1 视图**：
  - 设计（逻辑/结构视角）→ 类图、对象图；
  - 实现（开发视角）→ 组件图；
  - 过程（行为视角）→ 序列图、协作图、活动图、状态图；
  - 部署（物理视角）→ 部署图；
  - 用例（场景视角）→ 用例图，连接以上四方面。
- **构建块**：UML 由基本构建块（语言单词）、组合规则（语法）、通用机制（语义）三部分组成。
- **关系**：依赖、关联、泛化、实现——把事物紧密联系起来。
- **图**：结构图（静态部分）与行为图（动态部分）；序列图与协作图同构、状态图与活动图同构，可相互转换而不丢失信息。

## 工作机制

### 软件过程中的 UML 应用

| 软件开发流程 | 常用 UML 图 | 应用场景 |
| --- | --- | --- |
| 需求分析 | 用例图 | 对参与者（Actor）与所需功能建模，表示用户需求 |
| 概要设计 | 类图、对象图、协作图、状态图、序列图、活动图 | 描述系统静态结构与动态结构 |
| 详细设计 | 类图、对象图、协作图、状态图、序列图、活动图 | 把概要设计扩展成技术解决方案 |
| 编码阶段 | / | 把设计类转换成面向对象语言代码 |
| 测试 | 类图、组件图、部署图 | 单元测试用类图，集成测试用组件图与部署图 |

### 面向对象分析与设计

1. 面向对象分析：识别对象并用适当方式描述。
2. 面向对象设计：重点放在需求与实现。
3. 使用面向对象语言实现（Java、C++ 等）。

## 示例或代码

- **图书管理系统**：见 [[example]] 用例。
- **常用工具**：PlantUML（[教程](https://plantuml.com/zh/)、[在线绘制](http://www.plantuml.com/plantuml/uml/SyfFKj2rKt3CoKnELR1Io4ZDoSa70000)）；mermaid（[教程](https://mermaid-js.github.io/mermaid/#/)、[在线绘制](https://mermaid.live/edit#pako:eNpVj82qwkAMhV8lZHUF-wJdCNre66agoLuOi9CmzqDzwzRFpO27O9WNN6uQ853DyYiNbxlzvEYKGs6lcpBmWxc6ml4s9RfIss20ZwHrHT8n2P3sPfTah2DcdfXhdwsExVgtGINo427zRyre_oPjCcq6oiA-XL6V88NP8Fubo07x_xUdObn-6o7yjrKGIhQU3wiu0XK0ZNpUfVwuCkWzZYV5WlvuaLiLQuXmhNIg_vR0DeYSB17jEFoSLg2lpy2m7HvP8wuLb1U-)）。

## 常见误区

- **“UML 是编程语言”**：不是。UML 是图形建模语言，但工具可以基于 UML 图生成各种编程语言的代码。
- **“UML 只属于某个开发过程”**：UML 独立于开发过程，是系统建模语言，不绑定特定流程。
- **“UML 只能画静态结构”**：UML 同时包含结构图（静态）与行为图（动态），能描述系统的静态与动态两方面。
- **“序列图与协作图互不相同、不可转换”**：二者是同构的，可以相互转换而不丢失任何信息；状态图与活动图同样如此。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| uml-audit-1 | web-computer-science-uml | 该 claim 为 PlantUML 官网推广文案（迁移期误锚），与 UML 概述主题无直接关联，按规则保留原文案 |
...
- [[software-modeling]] —— 软件建模总论，UML 是其核心建模语言。
- [[class-diagram]] / [[object-diagram]] / [[component-diagram]] / [[deployment-diagram]] —— 结构类图。
- [[use-case-diagram]] / [[sequence-diagram]] / [[collaboration-diagram]] / [[statechart-diagram]] / [[activity-diagram]] —— 行为类图。
