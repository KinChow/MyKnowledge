...
evidence:
- claim: CUDA 是 NVIDIA 开发的并行计算平台和编程模型，借助 GPU 提升计算性能。
  claim_id: cpp-concurrent-programming-and-cuda-audit-1
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-b98c8e4a7b27
    source_id: nvidia-cuda-programming-guide-v2
id: cpp-concurrent-programming-and-cuda
kind: knowledge
publication_scope: public
related: []
schema_version: wiki/v1
sources:
- nvidia-cuda-programming-guide-v2
- working-computer-science-c-concurrent-programming-and-cuda
status: published
tags:
- cpp
- concurrency
...
C++ 并发编程通过线程（std::thread）、互斥锁（std::mutex）、条件变量、原子操作等原语在 CPU 上实现多线程并发，并借助 RAII 管理线程与锁资源；CUDA 是 NVIDIA 开发的并行计算平台和编程模型，通过 GPU 把大量线程组织成 thread block/warp 并行执行，实现大规模数据并行加速。

## 核心概念

- **并发 vs 并行**：并发是逻辑上的同时发生（能处理多个同时性活动，不一定要同一时刻）；并行是物理上的同时发生（同时发生的两个并发事件，具有并发含义，而并发不一定并行）。
- **进程/线程/协程**：进程是系统资源调度分配的基本单位；线程是进程的子任务、CPU 调度分派的基本单位，共享同一地址空间；协程是用户态轻量级线程，调度完全由用户控制、切换开销小。
- **互斥与同步**：std::mutex 系列互斥对象、std::lock_guard/std::unique_lock/std::shared_lock 等 RAII 锁管理类、条件变量（condition variable）、原子操作（std::atomic）。
- **CUDA 并行模型**：Thread（并行基本单位）、Thread block/CTA（互相合作、可同步、通过快速共享内存交换数据）、Grid（一组 thread block）、Kernel（在 GPU 上执行的核心程序，one kernel ↔ one grid）。
- **SIMD vs SIMT**：SIMD 一条指令处理多个数据（Intel mmx/sse、Arm Neon，发生在 ALU 内部）；SIMT 一条指令多条线程（应用于 GPGPU，每个 thread 有独立状态寄存器、访存地址、执行路径）。

## 工作机制

- **std::thread**：线程对象拥有一个执行线程；不支持拷贝、支持移动（`std::thread t2(std::move(t1))`）；执行体可以是任意可调用对象（普通函数、lambda、重载 operator() 的对象、std::bind 等）。线程分可结合（joinable）与分离（detached）两种状态；可用 RAII 技法（如 thread_guard）防止忘记 join/detach。
- **死锁**：对多个互斥对象加锁可能死锁；解决办法：能否只用 1 个锁、加锁顺序一致、使用 `std::lock(mtx1, mtx2)` 同时加锁。
- **条件变量**：阻塞一个或多个线程直到收到通知、超时或发生虚假唤醒；notify_one/notify_all 类似脉冲信号，若 wait 在 notify 之后发生不会被唤醒，因此 wait 前需检查条件、notify 前需修改标识；使用 wait 前必须获得相应锁。
- **原子操作与内存序**：atomic 保证任意时刻只有一个线程访问资源，实现更接近底层、效率比互斥对象高；内存屏障确保特定操作顺序、影响数据可见性；volatile 不能解决多线程问题，只适合信号处理、内存映射硬件、setjmp/longjmp 等场合。
- **CUDA 硬件映射**：thread block 由程序员决定线程数，最终拆解为若干 warp（32 个线程），在 SM（streaming multiprocessor）中以 warp 或 half-warp 运行，同一（half-）warp 的线程齐头并进。
- **CUDA 开发流程**：cpu 原型实现 → 将数据传给显存 → kernel 函数 → 调用 kernel → 将数据结果传回内存。

## 示例或代码

RAII 管理线程回收（thread_guard）：
...
```cpp
std::thread t2(std::move(t1)); // std::thread 不支持拷贝，但支持移动
```

## 常见误区

- **把并发等同于并行**：并发是逻辑上的同时发生，并行是物理上的同时发生；并发不一定并行。
- **以为 volatile 能解决多线程问题**：volatile 不能解决多线程中的问题，只在信号处理、内存映射硬件、非本地跳转（setjmp/longjmp）三种场合合适。
- **以为条件变量只要 notify 就能唤醒**：如果 wait 调用发生在 notify 之后是不会被唤醒的，wait 前需要检查条件（标识），notify 前需要修改相应标识。
- **以为 C++11/Boost 提供 Semaphore**：C++11 和 Boost.Thread 都没有提供信号量，因为信号量太容易出错，组合 mutex + condition variable 更安全。
- **以为 SIMD 与 SIMT 等价**：SIMD 运行的程序可以 SIMT 运行，反之不一定；SIMD 发生在 ALU 内部，SIMT 应用于 GPGPU。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| cpp-concurrent-programming-and-cuda-audit-1 | nvidia-cuda-programming-guide-v2 | CUDA 是 NVIDIA 开发的并行计算平台和编程模型，借助 GPU 提升计算性能 |
...
- [[cpp-class-and-raii]]：RAII 管理线程与互斥锁资源
- [[cpp]]：C++ 语言综合问答
- [[cpp-overview]]：C++ 语言概述
- [[cpp-course-chapters]]：课程目录（含并行编程、线程、mutex 章节）
