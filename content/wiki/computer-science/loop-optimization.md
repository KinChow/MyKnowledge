...
evidence:
- claim: 'Vectorization is enabled by the flag

    -ftree-vectorize and by default

    at -O3.'
  claim_id: loop-optimization-audit-1
  support: direct
  supporting_quotes:
...
- web-computer-science-loop-optimization
- working-computer-science-loop-optimization
status: published
tags:
- compiler
- loop-optimization
...
循环级优化是对循环进行的一系列等价变换，包括循环不变量外提、循环展开与压紧、循环合并、循环分布、循环分段、循环交换、循环分块、循环分裂、循环倾斜等，通过减少循环开销、增强数据局部性、提高指令级并行与并行化/向量化机会来提升程序性能；所有变换都必须保证不改变源程序语义，且通常需要满足依赖关系等合法性条件。

## 核心概念

- **循环不变量**：在循环迭代空间内值不发生变化的变量，可外提到循环外仅计算一次。
- **循环展开（loop unrolling）**：牺牲程序尺寸来加快执行速度，将循环体代码复制多次，减少循环分支指令执行次数、增大指令调度空间、获得更多指令级并行；**循环压紧**将复制后的多条语句合并到一起。
- **循环完全展开**：按照迭代次数对循环进行展开。
- **循环合并（Loop Fusion）**：将具有相同迭代空间的两个循环合成一个循环。
- **循环分布（Loop Distribute）**：将一个循环分解为多个与原循环迭代空间相同的循环，常用于分解出可向量化/可并行化的循环。
- **循环分段（strip mining）**：将单层循环变换为两层嵌套循环，用分段因子 strip 划分连续区域。
- **循环交换（Loop Interchange）**：交换紧嵌套循环中两个循环的嵌套顺序（重排序变换）。
- **循环分块（loop tiling/blocking）**：通过增加循环嵌套维度、重新划分迭代空间来提升数据局部性，是循环交换与循环分段的结合。
- **循环分裂**：对循环的迭代次数进行拆分，拆成两段或多段。
- **循环倾斜**：改变迭代空间形式以挖掘循环并行潜能。

## 工作机制

- **循环不变量外提**：把循环体内不随迭代变化的值（如 `1/(t*t)`）计算外提到循环外，削弱计算强度；合法性要求变换不影响源程序语义。LLVM 通过 `-Rpass=licm` 观察该优化。
- **循环展开**：复制循环体以减少分支指令与增加指令级并行，需要处理尾循环（迭代次数不能被展开次数整除时）；过度展开可能增加寄存器压力、引起指令缓存溢出。LLVM 相关选项包括 `-funroll-loops`、`-no-unroll-loops`、`-mllvm -unroll-count` 等，也可用 `#pragma clang loop unroll(...)` 指示。
- **循环合并**：将相同迭代空间的两个循环合为一个，减小迭代开销、增强数据重用；合法性要求不能违反原来的依赖关系圈、不能产生新依赖。LLVM 中需要两个循环相邻、迭代次数相同、控制流等价、无负距离依赖。
- **循环分布**：把循环拆成多个子循环，将一个串行循环转变为多个并行循环、实现部分并行化；也可配合循环交换、循环合并使用。LLVM 用 `-mllvm -enable-loop-distribute`，GCC 用 `-ftree-loop-distribute`。
- **循环分段**：内层循环遍历 strip 大小的连续区域、外层循环步进为 strip，通常用于外层并行化、内层向量化；循环分段不改变迭代次序，总是合法。
- **循环交换**：交换紧嵌套循环顺序以增强数据局部性、向量化与并行化识别；合法性需通过循环依赖关系判定，不能引起依赖关系反转。
- **循环分块**：把迭代空间按块重新划分，使 cache line 在被淘汰前能被再次使用；分块前后要保证迭代空间相同。
- **向量化**：向量化由 `-ftree-vectorize` 标志开启，默认在 `-O3` 生效。
...
- **以为循环展开次数越多性能越高**：过度展开可能增加寄存器压力、引起指令缓存区溢出，并不总是更快。
- **以为所有循环都可以合并**：循环合并需要满足合法性要求（不能违反依赖关系圈、不能产生新依赖），否则会导致结果错误。
- **以为循环交换总是有效**：交换依赖端点会导致依赖关系反转，是非法的循环交换。
- **以为循环分布一定有利**：循环分布可能减小并行性粒度、增加额外通信与同步开销。
- **以为编译器会自动优化一切**：很多循环优化需要满足合法性条件，且由代价模型决定是否执行，开发人员需理解其适用场景。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| loop-optimization-audit-1 | gcc-auto-vectorization | 向量化由 `-ftree-vectorize` 标志开启，默认在 `-O3` 生效 |
