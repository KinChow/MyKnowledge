...
- claim: Khronos 的 OpenCL 页面说明，OpenCL 通过核心规范纳入已广泛部署的能力，并支持开发者依赖跨实现能力。
  claim_id: opencl-cross-platform
  support: direct
  supporting_quotes:
...
- working-computer-science-opencl
status: published
tags:
- opencl
- gpu
...
OpenCL 是把 CPU、GPU 等异构设备统一到单一平台编程的开放标准（并行编程框架），通过平台、执行、内存、编程四层抽象，由 host 程序经命令队列把 kernel 提交到设备执行；目标是统一“可移植”与“高效”。

## 核心概念

- **平台模型**：一个 host 链接一个或多个 device；device 划分为 compute unit（CU），CU 划分为 processing element（PE），计算在 PE 中进行。
- **执行模型**：kernel 在 device 上执行；计算由 work-group 中的 work-item 进行；NDRange 是 N（1/2/3）维索引空间，分解为 work-group 与 sub-group。
- **内存模型**：host memory 与 device memory（global/constant/local/private 四个命名地址空间）；memory object（buffer/image/pipe）；Shared Virtual Memory（SVM）。
- **编程模型**：平台层、runtime、compiler 三组件，把 host 与多个设备当作单个异构并行计算机系统。
- **Kernel 分类**：OpenCL kernel（OpenCL C/SPIR-V）、native kernel（host 函数指针）、built-in kernel（绑定特定设备）。

## 工作机制

1. host program 通过 OpenCL API 创建 context 与命令队列，把 kernel 作为命令提交到 device。
2. 命令经过六个状态：Queue → Submitted → Ready → Running → Ended → Complete；命令间通过 event object 通信。
3. 命令队列可 in-order 或 out-of-order；同一上下文可有多个命令队列，用事件对象定义同步点。
4. kernel 实例 = kernel + 参数值 + 索引空间参数；设备从 work-pool 调度 work-group 到 CU/PE 执行。
5. device 侧入队允许 parent kernel 入队 child kernel，实现嵌套并行而无需 host 参与。
6. 同步域分 work-group、sub-group、命令同步；OpenCL 2.x 引入原子操作、fence 与内存序/内存作用域控制的宽松内存模型。
7. 内存一致性：OpenCL 1.x 较宽松；2.x 基于 ISO C11，SVM 分粗粒度 buffer、细粒度 buffer、细粒度 system 三种。

## 示例或代码

二维索引空间示例（摘自原文）：
- work-items 总数 (Gx×Gy)；work-group 大小 (Sx, Sy)；work-group 数量 (Wx, Wy) = (ceil(Gx/Sx), ceil(Gy/Sy))。
- 全局 ID 与局部 ID 转换：(gx, gy) = (wx×Sx + sx + Fx, wy×Sy + sy + Fy)。

（完整叙述见 ## 详细章节。）

## 常见误区

- 误以为同一 work-group 内的 work-item 一定并行执行——规范只保证在 sub-group/device 支持下才独立执行，否则可能被序列化。
- 误以为 work-group 之间可安全同步——OpenCL 未定义 work-group 间的 forward-progress/顺序，work-pool 中无法安全可移植地同步。
- 误以为 built-in kernel 能用 kernel 语言编程——built-in kernel 绑定特定设备，不能从 program object 源码 build。
- 混淆四种命名地址空间与 generic address space 的指针转换规则。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| opencl-cross-platform | khronos-opencl-overview-v2 | OpenCL 3.1 将 field-proven 能力纳入核心规范（含 SPIR-V），跨实现可用 |
...
- [[overview-of-opencl]] —— OpenCL 概述与基础教程。
- [[cl-mem]] —— OpenCL 内存对象与分配方式。
- [[limitations-of-local-work-size-and-global-work-size]] —— local/global work size 限制。
