...
- working-computer-science-mali-gpu-overview
status: published
tags:
- mali
- gpu
...
Mali GPU 是 ARM 为移动设备设计的高能效 GPU 系列，历经 Utgard、Midgard、Bifrost、Valhall 等架构世代演进，广泛集成于海思、联发科、三星等 SoC；OpenCL 支持从 Utgard 的不支持，逐步到 Midgard 的 1.1/1.2、Bifrost/Valhall 的 2.0/3.0。

## 核心概念

- **Utgard**（Mali-200/300/400/450/470）：非统一 GPU（离散像素/顶点着色器），主要专注图形加速，OpenGL ES 2.0，不支持 OpenCL。
- **Midgard**（T6xx/T7xx/T8xx）：分层平铺系统；第 1 代 Mali-T604/T658、第 2 代 T678、第 3 代 T760、第 4 代 T860/T830/T820/T880；支持 OpenCL 1.1/1.2。
- **Bifrost**（G71/G72/G76）：四向量化统一着色器、标量 ISA、条款执行、完全缓存一致性；OpenCL 2.0。
- **Valhall**（G77/G57/G78/G710/G510/G310/G715/G615/G720/G620/Immortalis）：新型超标量引擎、简化标量 ISA、动态调度、CSF 前端、光线追踪、DVS 管道；OpenCL 2.0/3.0。
- **实现厂商**：HiSilicon、MediaTek、Samsung 等 SoC 集成（具体型号见 ## 详细章节表格）。

## 工作机制

各代架构在发布时间、核心数、二级缓存与 API 支持上逐步演进；Utgard 仅图形，Midgard 起引入 OpenCL，Bifrost 采用统一着色器与标量 ISA，Valhall 引入超标量引擎、动态调度与 CSF 前端，第 4/5 代加入光线追踪、可变速率着色与延迟顶点着色（DVS）。详细时间线与特点见 ## 详细章节。

## 示例或代码

（架构时间线与 HiSilicon/MediaTek/Samsung SoC 集成表格见 ## 详细章节。）

## 常见误区

- 误以为所有 Mali GPU 都支持 OpenCL——Utgard 不支持 OpenCL。
- 误以为同架构所有型号 API 支持完全一致——需按具体型号核对（如 Bifrost 第 3 代标注为“第四代”，原文为笔误）。
...
- 表中部分频率标注为 “???”（未知），待补充。
- 原文 “Sansung” 应为 Samsung；Bifrost 第 3 代“第四代”表述待核对。

## 关联知识

- [[mali-bifrost-gpu-compute]] —— Mali Bifrost 上的 OpenCL 计算。
