...
- claim: 'RAII
    ...
    heap memory, thread of execution, open socket, open file, locked mutex, disk space,
    database connection—anything that exists in limited supply) to the lifetime of
    an object.'
  claim_id: cpp-class-and-raii-audit-1
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-ba1aae8c21d5
    source_id: web-computer-science-c-class-and-raii
id: cpp-class-and-raii
kind: knowledge
publication_scope: public
related: []
schema_version: wiki/v1
sources:
- web-computer-science-c-class-and-raii
- working-computer-science-c-class-and-raii
status: published
tags:
- cpp
- raii
...
RAII（Resource Acquisition Is Initialization，资源获取即初始化）通过把资源的获取与对象的生命周期绑定——对象构造时获取资源、对象析构时自动释放资源——来保证内存等稀缺资源在程序的已知时间点被确定性释放，是 C++ 资源管理（含智能指针）的核心机制。

## 核心概念

- **RAII**：资源在对象初始化时获取，对象拥有的资源随对象超出作用域时的析构函数自动释放；"通过栈上对象的生命周期，控制资源的获取与释放"。
- **对象生命周期**：栈上对象由系统分配与回收（先进后出，地址向下增长）；堆上对象由用户用 new/delete（或 malloc/free）申请与释放。
- **特殊成员函数**：三之法则（析构/拷贝构造/拷贝赋值）、五之法则（+移动构造/移动赋值）、零之法则（不需要管理资源所有权就不应自定义）。
- **智能指针**：`shared_ptr`（共享所有权、引用计数）、`unique_ptr`（独占所有权、只能移动）、`weak_ptr`（不增加引用计数的弱引用，解决循环引用）。
- **移动语义**：左值/右值、`std::move`/`std::forward` 与引用折叠，实现资源所有权转移以提升性能。

## 工作机制

- **内存布局**：可执行文件由 `.text`/`.init`/`.fini`/`.data`/`.bss` 段构成；运行时进程用户空间分为栈（stack，系统管理、地址向下增长）、堆（heap，用户申请释放、地址向上增长）、bss/data/text 段。
- **new/delete 过程**：`new` 先调用 operator new 分配空间，再调用构造函数；`delete` 先调用析构函数，再调用 operator delete 释放空间。数组必须用 `delete[]` 才能正确调用所有元素的析构函数。
- **字节对齐**：结构体元素按自身对齐大小放置在其整数倍偏移上；结构体总大小对齐到最大元素对齐大小的整数倍；含虚函数的类会多出一个（多继承下多个）vptr 指针的空间。
- **移动语义**：`std::move` 是无条件把参数转成右值引用，`std::forward` 按实参原始值类别转发；引用折叠规则让 `T&&` 在函数模板中成为万能引用。移动后必须把源对象资源正确重置，保证其可被正常析构。
- **RAII 落地**：把裸指针用对象包装、声明于栈上，由系统管理资源，避免资源泄漏。
...
```cpp
// shared_ptr：共享同一块堆内存，use_count() 查看引用数量，reset() 释放所有权
std::shared_ptr<T> sp = std::make_shared<T>();
sp.use_count();

// unique_ptr：独占所有权，不能赋值给其他 unique_ptr，只能移动
std::unique_ptr<T> up = std::make_unique<T>();
std::unique_ptr<T> up2 = std::move(up);

// weak_ptr：不使 shared_ptr 引用计数 +1，配合 shared_ptr 解决循环引用
...
```cpp
// std::move 只是把左值转换为右值引用（无条件转换），本身不移动任何东西
std::string a = "hello";
std::string b = std::move(a); // 调用移动构造，资源所有权转移
```

## 常见误区

- **`new[]` 与 `delete[]` 不配对**：对 `new` 出来的数组使用 `delete` 只会删除第一个元素，必须用 `delete[]` 才能正确调用所有析构函数。
- **把 `const` 当作运行期不可改**：`const` 是编译期检查，用 `const_cast` 或 C 风格强转依然能修改其只读属性；对全局常量进行 `const_cast` 后赋值是未定义行为。
- **在构造函数/析构函数中调用虚函数**：会导致未实现多态的行为；基类一次只构造一个完整对象。
- **在移动构造/移动赋值中不重置源对象**：移动后应把源对象资源正确重置，否则源对象析构时可能释放已被移动的资源；不要依赖已 move 对象的值。
- **把派生类对象直接赋值给基类对象**：会发生切片（只拷贝/移动基类部分），损害多态行为。
- **通过基类指针释放派生类时基类析构函数不是虚函数**：只会调用基类析构函数，不调用派生类析构函数，导致内存泄漏。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| cpp-class-and-raii-audit-1 | web-computer-science-c-class-and-raii | RAII 把资源（堆内存、线程、socket、文件、锁等）的生命周期绑定到对象生命周期 |
...
- [[cpp]]：C++ 语言综合问答（指针/引用、内存管理、STL 等）
- [[cpp-concurrent-programming-and-cuda]]：多线程与 RAII 管理锁/线程资源
- [[cpp-vs-java]]：C++ 手动内存管理与 Java 自动 GC 的对比
- [[cpp-vs-c]]：C 语言与 C++ 在 struct/内存管理上的区别
