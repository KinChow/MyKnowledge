...
- claim: Khronos 的 OpenCL 页面说明，OpenCL 通过核心规范纳入已广泛部署的能力，并支持开发者依赖跨实现能力。
  claim_id: mali-opencl-cross-platform
  support: direct
  supporting_quotes:
...
id: optimizing-opencl-for-mali-gpus
kind: knowledge
...
- working-computer-science-optimizing-opencl-for-mali-gpus
status: published
tags:
- opencl
- mali
...
优化 Mali GPU 上的 OpenCL 应用，关键是：先定位最耗时的内核并逐个测量优化；让控制线程与 OpenCL 线程并行（避免 clFinish() 序列化）；用 OpenCL API（CL_MEM_ALLOC_HOST_PTR）分配内存以避免拷贝，并利用共享内存/一致性系统降低内存开销。

## 核心概念

- **优化步骤**：识别最密集内核 → 单独测量（多次运行取平均）→ 分析（读写次数/计算密度，可用 Offline Shader Compiler）→ 隔离瓶颈 → 应用优化 → 测量检查 → 迭代。
- **负载均衡**：避免 clFinish() 同步，用 clWaitForEvents()/回调；不要把 clEnqueueMap() 与阻塞调用一起用。
- **内存分配标志**：CL_MEM_ALLOC_HOST_PTR（首选，避免拷贝）、CL_MEM_COPY_HOST_PTR（复制 host 数据）、CL_MEM_USE_HOST_PTR（慎用，可能强制内存限制）；不要用 malloc()。
- **一致性系统**：完全一致性与 I/O 一致性系统可减少/免除 cache clean/invalidate。
- **带宽/计算权衡**：带宽受限 → 用计算代替读取；计算受限 → 用读取代替计算。

## 工作机制

1. Mali 系统通常有共享内存，但 OpenCL 假设内存独立，缓冲区分配默认涉及拷贝，耗时耗电。
2. CL_MEM_ALLOC_HOST_PTR 让应用处理器与 Mali GPU 无拷贝访问数据；CL_MEM_USE_HOST_PTR 可能在首次使用或 map 时触发拷贝。
3. I/O 一致性系统下用 CL_MEM_ALLOC_HOST_PTR 启用 I/O 一致分配，免去 cache clean/invalidate。
4. 完全一致性系统允许 CPU/GPU 直接共享数据，无需对 memory object 做 cache 操作。

## 示例或代码

（cl_mem_flags 对比表格与示意图片见 ## 详细章节。）

## 常见误区

- 用 clFinish() 做同步——它序列化执行并引入延迟。
- 用 malloc() 分配内存——Mali GPU 可能无法访问（页未映射到 GPU 地址空间）。
- 把 CL_MEM_USE_HOST_PTR 当默认选择——可能强制内存限制并降低性能。
- 依赖重复 map 返回同一地址——应始终使用返回的最新指针。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| mali-opencl-cross-platform | khronos-opencl-overview-v2 | OpenCL 3.1 核心规范纳入 field-proven 能力（含 SPIR-V） |
