...
evidence:
- claim: Arm NEON 可用于多媒体、信号处理和图像处理等对定点或浮点性能敏感的场景。
  claim_id: arm-software-optimization-neon
  support: direct
  supporting_quotes:
...
sources:
- arm-neon-intro
- working-computer-science-arm-software-optimization
status: published
tags:
- arm
- performance
...
Arm 软件性能优化需要自顶向下做端到端分析：从芯片层次、存储层次、流水线、NUMA、PMU 等硬件基础出发，按业务级、系统架构级、算法级、代码级、编译器五个层次寻找并消除瓶颈；所有软件手段从硬件角度看不外乎内存读写优化、cache 命中率优化、流水线优化与计算能力最大化四类。

## 核心概念

- **芯片层次**：CCL（CPU cluster，含多个 CPU core）→ SCCL（Super CPU cluster/Die，含多个 CCL）→ Chip（socket，含多个 SCCL，chip 间经 Hydra 接口连接）。
- **存储层次时延**：序号越大访问时延越大——L1 4 cycle、L2 8 cycle、L3 38 cycle、DDR local 88ns、DDR cross die 105ns、DDR remote 192~230ns。
- **流水线**：Execution Throughput 为每 cycle 最大执行指令数；复制流水线结构并行执行即超标量流水线。冲突分结构冲突、数据冲突（RAW/WAR/WAW）、控制冲突；数据/结构冲突影响局部，控制冲突影响全局。
- **NUMA**：非统一内存访问架构，CPU 分多个节点（Node），各节点有自己的内存 Controller 与内存插槽，节点间经片内/片间高速总线访问。
- **PMU**：ARM CPU 内的性能监控单元，对 CPU 事件计数，perf 即通过读取 PMU 事件进行性能分析。
- **四个硬件优化维度**：内存读写优化、cache 命中率优化、流水线优化、计算能力最大化（单核用向量/原子指令，多核并行化）。

## 工作机制

1. 自顶向下分层：业务级（软硬件资源配置、负载均衡）→ 系统架构级（模块划分、数据流、约束）→ 算法级（时间/空间复杂度）→ 代码级（符合硬件运行规律的编程技巧）→ 编译器（自动优化与编译选项指导）；各环节优化不能相互冲突（避免 1+1<2）。
2. 发现瓶颈：依靠代码检视、perf 等工具定位性能瓶颈。
3. 逐项应用优化手段：cache 优化（cacheline 对齐、消除伪共享、数据重排、软件预取、non-temporal load/store）→ 结构体优化（字节对齐、调整成员顺序）→ 循环优化（展开/融合/分离/置换/平铺）→ 分支优化（分支预测、查表、switch）→ 函数优化（参数传递、内联）→ 指令优化（向量化、减少高开销指令、重排序、替换、减少 barrier）→ 访存优化（减少内存读写/分配、合理使用全局变量）→ 多核优化（锁优化、减少跨 NUMA 访问）→ 语言特性（移动语义、constexpr 等）。
4. 验证并避免过度优化：如 barrier 能不用就不用，锁粒度不是越小越好。

## 示例或代码

- **编译选项**：`-free-vectorize` 或 `-O3` 开启自动向量化（GCC 2008 起，`-O3` 默认打开）；`-O0`~`-O3` 四档优化级别。
- **软件预取**：GCC `__builtin_prefetch(addr, rw, locality)`，rw 0 读/1 写，locality 0~3 表示访问频率。
- **字节对齐**：`__attribute__((packed))`、`__attribute__((aligned(4)))`。
- **分支优化**：`likely()`/`unlikely()` 提升分支预测准确性；避免循环内判断；查表减少分支。
- **伪共享消除**：OpenMP reduction 子句、线程私有变量按 cacheline 对齐。
- **NUMA 绑定**：`numactl -H`、`numactl -C 0-7 test`、`numastat`；OpenMP 用 `OMP_PROC_BIND=true` 与 `OMP_PLACES` 绑定核。
- **锁优化**：LL/SC（ldaxr/stlxr）与 LSE（ldaddal）；GCC 加 `-march=armv8-a+lse` 等选项。
- **Barrier**：ARMv8 提供 DMB、DSB、ISB，执行代价依次递增，能用 DMB 不用 DSB/ISB。
- **工具**：perf、ftrace（`trace-cmd record -p function_graph -P 25314`）、strace（`-c`/`-T` 查看统计与耗时）、ltrace、top、pstack。

## 常见误区

- **只做底层代码优化**：算法层面的优化相比底层代码优化往往能带来更大收益，应优先考虑算法优化。
- **误用大页表**：大页表能降低 TLB miss、减少 TLB walk 延迟，但容易带来内存使用效率降低。
- **过度使用 barrier**：能不用 barrier 尽量不用；能用 DMB 不要用 DSB/ISB（执行代价递增）。
- **锁粒度误区**：锁粒度不是越小越好——太大会增加撞锁概率，太小会导致代码更难写。
- **盲目 switch**：case 值跳跃性太大会使编译器把 switch-case 退化成 if-else；switch 适用于连续的 enum 成员比较。
- **忽视流水线冲突**：汇编级优化主要针对结构、数据、控制三类冲突做流水线优化。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| arm-software-optimization-neon | arm-neon-intro | Arm NEON 可用于多媒体、信号处理、3D 图形、语音、图像处理等对定点/浮点性能敏感的场景 |
...
- [[cpu-cache-optimization]]：cache 命中率优化
- [[cpu-pipeline-and-hazards]]：流水线与冲突
- [[arm-neon]]：Arm NEON 向量指令
- [[multi-core-software-design]]：多核软件设计
- [[compiler-vectorization]]：编译器自动向量化
- [[computer-performance-and-power-consumption]]：计算机性能与功耗
...
从2006年左右开始，所有的PC、服务器的处理器，都迈入了多核时代。这时出现了第一代多核架构，称为SMP：也就是对称多处理器架构。在SMP架构下的计算机中，每个核都是对等的，所有的核通过总线访问所有内存，每个进程在调度时，可以在任意一个核上运行，在操作系统和内核的支持下，整个系统能做到非常好的负载均衡，性能得到很好的发挥。
但是，我们也看到了一个问题。所有的核均通过总线访问内存，且共用一个控制器，当核数不断增加的时候，内存总线和单一控制器成为了内存访问的瓶颈。为了解决这一问题，NUMA架构出现了。
NUMA架构也叫非统一的内存访问架构（Non-uniform memory access），下图是一个NUMA架构的示例，这里有两个CPU，在NUMA架构下，CPU被分成了多个节点Node。每个节点有自己的内存Controller，也有自己的内存插槽，这个内存的访问不再受一条内存总线带宽的限制。NUMA node之间的内存访问，通过片内的和片间的高速总线完成。
...
* 编译器的优化包括编译器本身对代码的自动优化，也包括程序员通过编译选项指导编译器进行优化。
...
* -O2：推荐编译选项，进行更多优化，但是不会进行循环展开和函数内联；
...
cpu访问数据是按cacheline访问的，对于关键的热点数据，在分配内存时尽量按cacheline对齐。L1/L2 cache的cachline大小为64字节，L3 cache的cacheline字节为128字节，跨越cpu cluster的共享内存需要按L3 cacheline对齐。
...
多分支条件语句一般采用switch语句，这样的程序无论从清晰性和效率上都比原来的程序要好。因为switch-case语句实际上会生成一个连续的查找表，在查找对应case，均只需要一次比较，而if-else需要每个条件都计算一遍。实际编程中，当分支比较少时，使用if-else即可，当分支比较多，使用switch-case。需要注意的是，由于switch-case的查找表大小为case最大值与最小值之间的间距，如果case条件值跳跃性太大，编译器会将switch-case退化成if-else。所以switch-case通常用于多个连续的enum类型的成员的比较。
...
现代CPU为了提升流水线的执行效率，一般都实现了乱序执行的功能，意味着代码的指令顺序与指令实际执行顺序可能不一样，特别是没有寄存器依赖的两条指令，执行顺序无法确定。但有些场景，我们希望这两条指令是保序的，比如往某个内存地址写入一个值后，需要等到写入完成后（写入内存是需要时间的），再往某个外设寄存器中写入数据，因为这个寄存器地址与内存地址没有数据依赖，CPU无法保证执行顺序，所以提供了 barrier指令供程序员手动进行保序。



ARMv8提供3条Barrier指令：DMB、DSB、ISB。
* DMB：数据存储器隔离。DMB 指令保证：仅当所有在它前面的memory访问操作都执行完（不对非memory访问操作阻塞），才执行在它后面的指令。
* DSB：数据同步隔离。比 DMB严格：仅当所有在它前面的memory访问、device访问、cache维护等操作都执行完毕后，才执行在它后面的指令。
...
线程/进程实际运行所在的物理核与内存NUMA Node位置关系，会带来访存路径时延的差异，本地访问性能最佳，本Socket内跨一次NUMA次之，跨Socket访问性能最不理想。
