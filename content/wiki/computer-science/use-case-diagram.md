...
- claim: "New! Render PlantUML diagrams directly inside GitHub\n        with our official
  ...
  claim_id: use-case-diagram-audit-1
  support: direct
  supporting_quotes:
  ...
  targets:
  - evidence_id: evidence-e446160d9a06
    source_id: web-computer-science-use-case-diagram
id: use-case-diagram
kind: knowledge
...
- working-computer-science-use-case-diagram
status: published
tags:
- uml
- use-case-diagram
...
用例图由**参与者（Actor）、用例（Use Case）及其关系**组成，代表系统的**用例视图**：用于对应用程序的功能建模，收集系统需求、获取外部视图，描述谁使用系统以及他们能用系统做什么。

## 核心概念

- **定义**：用例图由参与者、用例及其关系组成；用例与参与者之间的对应关系又称通信关联（Communication Association）。
- **定位**：用例图用于对应用程序的系统/子系统功能建模，代表系统的用例视图；单个用例图捕获系统的特定功能。
- **参与者（Actor）**：与系统交互的用户角色，可以是人、组织、机器或另一个外部系统；用火柴人表示。
- **用例（Use Case）**：外部可见的系统功能，是对系统提供服务的描述；用椭圆表示。
- **子系统**：已建立构造型的组件，表示系统中独立的行为单元，由用例组成；通过包（方框 + 系统名称）表示。
- **关系**：关联（参与者-用例）、泛化（参与者之间或用例之间）、包含（`<<include>>`）、扩展（`<<extend>>`）。
...
| 关系 | 含义 | 表示 |
| --- | --- | --- |
| 关联 | 参与者与用例之间的关系 | 箭头 + 实线，参与者指向用例 |
| 泛化 | 子代接收父代的所有属性/操作/关系 | 三角箭头 + 实线，子代指向父代 |
| 包含 | 基本用例包含另一用例的功能，支持复用（use） | `<<include>>` + 箭头 + 虚线，基础用例指向被包含用例 |
| 扩展 | 在特定条件下把新行为加入已有用例，扩展用例指向基础用例 | `<<extend>>` + 箭头 + 虚线，扩展用例指向基础用例 |

### 何时使用包含关系

- 多个用例用到同一段行为，可把共同行为抽象成独立用例，让其他用例包含它。
- 某个用例功能过多、事件流过于复杂时，把某段事件流抽象成被包含用例以简化描述。
- 优点：提高可维护性，避免重复描述与描述不一致。
...
- 借阅者借书（参与者与用例的关联）。
- 不同类型的借阅者（老师、学生）借阅书本数量与最大借阅时长可不同（参与者之间的泛化）。
- 借书有两种方式：网上预约借书、现场借书（用例之间的泛化）。
- 借阅图书之前需要查询图书（包含关系）。
- 预约图书是借阅图书的一种扩展（扩展关系）。

绘制工具参考：https://plantuml.com/zh/use-case-diagram

## 常见误区

- **“用例从系统自身角度提取”**：用例需要从参与者希望系统提供的功能中提取，满足参与者需求的才是好用例。
- **“参与者只能是真人”**：参与者可以是人、组织、机器或另一个外部系统；单个参与者可表示多个用户，单个用户可有多个参与者角色。
- **“包含与扩展可以混用”**：包含（`<<include>>`）是基本用例必然用到被包含用例（复用）；扩展（`<<extend>>`）是在特定条件下可选地补充基础用例。
- **“用例要描述实现细节”**：用例描述系统外部可见的功能，并不描述如何实现的详细信息。

## 证据映射

| claim_id | 来源 | 要点 |
| --- | --- | --- |
| use-case-diagram-audit-1 | web-computer-science-use-case-diagram | 该 claim 为 PlantUML 官网推广文案（迁移期误锚），与用例图主题无直接关联，按规则保留原文案 |
...
- [[uml]] —— UML 总论，用例图属行为图（用例视角）。
- [[software-modeling]] —— 软件建模中的需求分析建模。
- [[class-diagram]] —— 把用例分析结果映射到类的静态结构。
