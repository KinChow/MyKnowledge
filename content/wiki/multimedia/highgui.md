...
- 原始位置：`content/working/c-intermediate/multimedia/highgui.md`
- 原文快照：`working-multimedia-highgui`（personal-note；用于保留作者原始整理，不等同于外部权威来源）
- 外部链接：2 条；链接仅作为待补来源线索，尚未自动认定为事实依据。
- 当前状态：`draft`；事实正确性、来源逐条对应和必要的权威来源补齐待人工审查。

### 来源补充

- 已联网抓取来源：`https://docs.opencv.org/4.x/d7/dfc/group__highgui.html`；正文已保存为不可变 Source 快照，可继续做逐条 evidence 锚定。
