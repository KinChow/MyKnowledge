...
class VaultRegistry:
    ...
    def resolve_vault_path(self, vault_id: str) -> Path:
        ...
        workspace = (
            self.root
            if data.get("workspace_root") in (None, "")
            else Path(data["workspace_root"]).expanduser().resolve()
        )
            ...
            for object_type, base in RepoPaths(path).object_roots:
                for item in sorted(base.rglob("*.md")) if base.is_dir() else []:
                    ...
                    if key in seen_objects:
                        conflicts.append(
                            {"code": "duplicate_object_id", "object_ref": ref}
                        )
                        affected.append(ref)
                    else:
    ...
    def validate_reference(
        ...
        target = (
            owner if target_vault_id in (None, "") else safe_id(str(target_vault_id))
        )
        result = {
            "valid": False,
            "code": None,
            "owner_vault_id": owner,
            "target_vault_id": target,
            "object_ref": {
                "vault_id": target,
                "object_type": object_type,
                "object_id": object_id,
        ...
        report = self.check()
        status = next(
            (item for item in report["vaults"] if item["vault_id"] == target), None
        )
        if status is None:
        ...
        if status["state"] != "available":
            ...
            return result
        if any(
            conflict.get("object_ref") == result["object_ref"]
                ...
                for path in sorted(base.rglob("*.md")) if base.is_dir() else []:
                    ...
                    if key in index:
                        index[key] = {
                            ...
                            "availability_reason": "duplicate_object_id",
                        }
                        continue
                    index[key] = {
                        "vault_id": vault_id,
    ...
    def local_projection(self, scope: str = "local") -> dict:
        ...
        items.sort(
            key=lambda item: (item["vault_id"], item["object_type"], item["object_id"])
        )
        unavailable = [
    ...
    def write_local_projection(
        ...
        target = (
            ...
            else RepoPaths(self.root).queries_local / "manifest.json"
        )
    ...
    def effective_confidentiality(
        owner_confidentiality: str, upstream_confidentialities: list[str] | None = None
    ) -> str:
        ...
        return (
            "internal"
            if max((levels.get(value, 1) for value in values), default=0)
            else "public"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        print(
            json.dumps(
                VaultRegistry(args.root, args.manifest).check(),
                ensure_ascii=False,
                indent=2,
            )
        )
