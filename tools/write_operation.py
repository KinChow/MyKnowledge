...
def _write_retire_marker(service: "WriteOperation", record: dict, operation_id: str) -> None:
    ...
    atomic_write(service.store.paths.audit_retire / f"{operation_id}.json", canonical_json(marker) + b"\n", 0o600)
...
class WriteOperation:
    ...
    def apply(self, operation_id: str, *, confirmed: bool = False, actor_id: str = "local-user", confirmation: dict | None = None) -> dict:
        record, error = self.store.apply_preflight(operation_id, OPERATION_TYPES, confirmed)
        ...
        try:
            with VaultLock(self.root, vault_id, operation_id) as lock:
                ...
                originals, pre_error = self._collect_originals(record, vault_id, operation_id)
                if pre_error is not None:
                    return pre_error
                intent_path = self._commit_files(record, vault_id, operation_id, lock, originals)
                ...
                return self._finalize(record, operation_id, actor_id, confirmation_record, applied_files, recovered=False, intent_path=intent_path)
    ...
    def _collect_originals(self, record: dict, vault_id: str, operation_id: str) -> tuple[dict[Path, bytes | None], dict | None]:
        ...
        if record.get("operation_type") == "rename" and record.get("source_path"):
            ...
            if not source_path.exists():
                ...
                return originals, {"state": "expired", "operation_id": operation_id, "error_code": "path_unresolved"}
            if record.get("source_before_hash") and sha256_bytes(source_path.read_bytes()) != record["source_before_hash"]:
                ...
                return originals, {"state": "expired", "operation_id": operation_id, "error_code": "hash_mismatch"}
        ...
        for item in record.get("files", []):
            ...
            if path.exists() and path.stat().st_nlink > 1:
                ...
                return originals, {"state": "expired", "operation_id": operation_id, "error_code": "path_hardlink"}
    ...
    def _commit_files(self, record: dict, vault_id: str, operation_id: str, lock: VaultLock, originals: dict[Path, bytes | None]) -> Path:
        ...
        try:
            for index, item in enumerate(record["files"]):
                ...
                lock.assert_owner()
                policy.commit_file(self._operation_path(item["path"], vault_id), item["content"])
    ...
    def _finalize(self, record: dict, operation_id: str, actor_id: str, confirmation_record: dict, applied_files: list[str], *, recovered: bool, intent_path: Path | None = None) -> dict:
    ...
    def recover(self, operation_id: str, *, projection_rebuilder: Callable[[dict], object] | None = None) -> dict:
        ...
        try:
            ...
            if record.get("state") == "applied_index_pending":
                ...
                return self._cleanup_intent_after_applied(applied, operation_id, recovered=True)
            intent_path = self.store.paths.commit_intent_file(operation_id)
