...
def replay(citation: dict[str, Any], snapshot: str) -> dict[str, Any]:
    ...
    try:
        ...
        if selector.get("type") != "TextQuoteSelector" or not isinstance(position, dict) or position.get("type") != "TextPositionSelector":
            raise ValueError("selector_unresolved")
        start, end = position.get("start"), position.get("end")
        if not isinstance(start, int) or not isinstance(end, int) or start < 0 or end <= start or end > len(snapshot):
            raise ValueError("selector_unresolved")
        exact = selector.get("exact")
        if not isinstance(exact, str) or snapshot[start:end] != exact:
            raise ValueError("selector_unresolved")
        if citation.get("quote_sha256") and citation["quote_sha256"] != sha256_text(canonical_quote(exact)):
            raise ValueError("selector_hash_mismatch")
        computed = hash_canonical({"snapshot_sha256": expected_snapshot, "start": start, "end": end, "exact": exact, "prefix": selector.get("prefix", ""), "suffix": selector.get("suffix", "")})
        if citation.get("selector_sha256") and citation["selector_sha256"] != computed:
            raise ValueError("selector_hash_mismatch")
        return {"state": "valid", "snapshot_sha256": expected_snapshot, "start": start, "end": end, "exact": exact}
