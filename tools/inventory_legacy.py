...
def _tree_hash(files: list[Path], root: Path) -> str:
    entries = [
        {"path": str(p.relative_to(root)), "sha256": sha256_bytes(p.read_bytes())}
        for p in files
    ]
...
def inventory(root: Path, docs_dir: Path | None = None) -> dict:
    ...
    source = (docs_dir or root / "docs").resolve()
    supported = {".md", ".html", ".htm", ".pdf", ".txt", ".docx"}
    files = sorted(
        [
            p
            for p in source.rglob("*")
            if p.is_file() and not p.is_symlink() and p.suffix.lower() in supported
        ],
    ...
    for path in files:
        ...
        body = (
            path.read_text(encoding="utf-8", errors="replace") if not is_binary else ""
        )
        title = next(
            (
                line.lstrip("# ").strip()
        ...
        shape = (
            ...
            if not nonempty
                ...
                or (heading_count <= 1 and len(body) < 500)
                else "article"
            )
        )
        ...
        extractor = (
            ...
            if path.suffix.lower() == ".docx"
                ...
                else ("trafilatura" if "html" in media_type else "utf8")
            )
        )
        items.append(
            {
                "legacy_path": str(path.relative_to(root)),
                "body_sha256": sha256_bytes(path.read_bytes()),
                ...
                "extractor": extractor,
                "route": "/" + str(path.relative_to(source)).rsplit(".", 1)[0],
                "source_target": None,
                "wiki_target": None,
    ...
    result = {
        "schema_version": "migration-inventory/v1",
        "generated_at": time.time(),
        "input_tree_sha256": _tree_hash(files, root),
        "classifier_version": CLASSIFIER_VERSION,
        "thresholds": {"index_max_bytes": 500},
        "items": items,
    }
    result["inventory_sha256"] = (
        "sha256:"
        + hashlib.sha256(
            canonical_json({k: v for k, v in result.items() if k != "inventory_sha256"})
        ).hexdigest()
    )
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Build legacy docs migration inventory"
    )
    parser.add_argument("--root", type=Path, default=Path.cwd())
    parser.add_argument("--docs", type=Path)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args(argv)
    result = inventory(args.root, args.docs)
