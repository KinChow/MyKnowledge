...
class IndexBuilder:
    ...
    def build(self, items: list[dict], scope: str = "local") -> dict:
        ...
        for x in allowed:
            rec = {
                "object_ref": {
                    "vault_id": x.get("vault_id"),
                    "object_type": x.get("object_type", "wiki"),
                    "object_id": x.get("object_id"),
                },
                "title": x.get("title"),
                "body": x.get("body")
                if x.get("availability", "available") == "available"
                else None,
                "snippet": None,
                "score": None,
                "availability": x.get("availability", "available"),
                "availability_reason": x.get("availability_reason", "none"),
                "confidentiality": x.get("confidentiality", "public"),
        ...
        return {
            ...
            "generated_from": hash_canonical(allowed),
            "availability": "available",
            "availability_reason": "none",
            "degraded": False,
            "confidentiality_max": "internal"
            if any(x["confidentiality"] == "internal" for x in records)
            else "public",
            "limits": [],
            "warnings": [],
        }
...
class SQLiteIndex:
    ...
    def rebuild(self, items: list[dict], scope: str = "local") -> dict:
        ...
        try:
            ...
            db.execute(
                "CREATE TABLE index_info (scope TEXT NOT NULL, generated_from TEXT NOT NULL, tokenizer TEXT NOT NULL)"
            )
            db.execute(
                "INSERT INTO index_info(scope,generated_from,tokenizer) VALUES(?,?,?)",
                (scope, generated_from, tokenizer),
            )
            db.execute(
                "CREATE TABLE metadata (rowid INTEGER PRIMARY KEY, object_ref TEXT, title TEXT, body TEXT, availability TEXT, availability_reason TEXT, confidentiality TEXT, content_sha256 TEXT, source_ref TEXT)"
            )
            db.execute(
                f"CREATE VIRTUAL TABLE documents USING fts5(title, body, content='metadata', content_rowid='rowid', tokenize='{tokenizer}')"
            )
            for row in allowed:
                ref = json.dumps(row["object_ref"], ensure_ascii=False, sort_keys=True)
                db.execute(
                    "INSERT INTO metadata(object_ref,title,body,availability,availability_reason,confidentiality,content_sha256,source_ref) VALUES(?,?,?,?,?,?,?,?)",
                    (
                        ref,
                        row["title"],
                        row["body"],
            ...
            db.execute("INSERT INTO documents(documents) VALUES('rebuild')")
            db.commit()
            db.close()
    ...
    def recover(self, items: list[dict], scope: str = "local") -> dict:
        ...
        if self.path.exists():
            try:
                ...
                row = db.execute(
                    "SELECT scope, generated_from FROM index_info LIMIT 1"
                ).fetchone()
    ...
    def search(self, query: str, top_k: int = 8) -> list[dict]:
        ...
        try:
            ...
            try:
                ...
                pass
            if tokenizer == "simple" and self._load_simple(
                db, self.root or self._infer_root()
            ):
            ...
            rows = db.execute(
                "SELECT m.object_ref,m.title,m.body,bm25(documents),m.availability,m.availability_reason,m.confidentiality,m.content_sha256,m.source_ref FROM documents JOIN metadata m ON documents.rowid=m.rowid WHERE documents MATCH ? ORDER BY bm25(documents) LIMIT ?",
                (match_expr, top_k),
            ).fetchall()
        finally:
            db.close()
        return [
            {
                "object_ref": json.loads(r[0]),
                "title": r[1],
                "snippet": (r[2] or "")[:240],
                "score": float(r[3]),
                "availability": r[4],
                "availability_reason": r[5],
                "confidentiality": r[6],
                "content_sha256": r[7],
                "source_ref": r[8],
            }
    ...
    def generated_from(self) -> str:
        ...
        try:
            return str(
                db.execute("SELECT generated_from FROM index_info LIMIT 1").fetchone()[
                    0
                ]
            )
        finally:
...
class Retriever:
    ...
    def search(
        ...
        if (
            ...
            or top_k > 100
            or (
                vault_ids is not None
                and (
                    len(vault_ids) > 16
                    or any(not isinstance(v, str) or not v for v in vault_ids)
                )
            )
        ):
            return {
                ...
                "availability_reason": "query_limit_exceeded",
                "degraded": True,
                "confidentiality_max": "public",
                "limits": ["query_limit_exceeded"],
                "warnings": [],
            }
        ...
        if self.index_path and self.index_path.exists():
            try:
                ...
                return {
                    ...
                    "method": "fts5",
                    "index_version": "fts5/v1",
                    "generated_from": hash_canonical(public),
                    "availability": "available",
                    "availability_reason": "none",
                    "degraded": False,
                    "confidentiality_max": "internal"
                    if any(x.get("confidentiality") == "internal" for x in indexed)
                    else "public",
                    "limits": [],
                    "warnings": [],
                }
        ...
        hits = [
            ...
            or (
                x.get("availability", "available") == "available"
                and q in str(x.get("body", "")).casefold()
            )
        ]
        result_items = []
        for x in hits[:top_k]:
            ...
            result_items.append(
                {
                    ...
                    "title": x.get("title"),
                    "snippet": str(x.get("body", ""))[:240] if available else None,
                    "score": 1.0,
                    "availability": "available" if available else "unavailable",
                    "availability_reason": "none"
                    if available
                    else x.get("availability_reason", "unavailable"),
                    "confidentiality": x.get("confidentiality", "public"),
                    "content_sha256": x.get("content_sha256"),
                    "source_ref": x.get("source_ref"),
                }
        ...
        return {
            ...
            "scope": scope,
            "method": "deterministic-fallback",
            "index_version": "fallback/v1",
            "generated_from": hash_canonical(public),
            "availability": "available",
            "availability_reason": "none",
            "degraded": True,
            "confidentiality_max": "internal"
            if any(x.get("confidentiality") == "internal" for x in public)
            else "public",
            "limits": [],
            "warnings": ["fts5_unavailable"],
        }
